function output=CoreFHorzExpAssetU_d1_noz_noe_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% ExpAsset noa1 + semiz: with d1 (labour), no z, no e. Experience asset is the only endogenous state.
...
ReturnFn=@(d1,d2,d3,a,semiz,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_noz_noe_noa1_semiz(d1,d2,d3,a,semiz,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);

FnsToEvaluate.assets=@(d1,d2,d3,a,semiz) a;
FnsToEvaluate.earnings=@(d1,d2,d3,a,semiz,w,kappa_j) w*kappa_j*d1*d2*a*semiz;
