function output=CoreFHorzExpAssetU_d1_z_noe_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% ExpAsset noa1 + semiz: with d1 (labour), z, no e. Experience asset is the only endogenous state.
...
% PENDING TOOLKIT SUPPORT: ExpAssetU+SemiExo+noa1 errors immediately at ValueFnIter call
...
FnsToEvaluate.assets=@(d1,d2,d3,a,semiz,z) a;
FnsToEvaluate.earnings=@(d1,d2,d3,a,semiz,z,w,kappa_j) w*kappa_j*d1*d2*a*z*semiz;
...
fprintf('noa1 moments (d1+semiz+z)\n')
...
title('Age-conditional assets (d1+semiz+z, noa1)')
