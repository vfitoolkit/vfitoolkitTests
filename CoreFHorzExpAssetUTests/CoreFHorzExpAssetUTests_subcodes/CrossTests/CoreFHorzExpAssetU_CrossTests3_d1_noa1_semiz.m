function output=CoreFHorzExpAssetU_CrossTests3_d1_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% Cross test 3 noa1+semiz (d1): ExpAssetU+noa1+semiz with degenerate u (n_u=1, prob 1) should equal
...
fprintf('Cross test 3 (noa1+d1+semiz): expassetu+semiz with u==1 reduces to expasset+semiz, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V0(:)-V1(:))),max(abs(Policy0(:)-Policy1(:))),max(abs(StationaryDist0(:)-StationaryDist1(:))))
