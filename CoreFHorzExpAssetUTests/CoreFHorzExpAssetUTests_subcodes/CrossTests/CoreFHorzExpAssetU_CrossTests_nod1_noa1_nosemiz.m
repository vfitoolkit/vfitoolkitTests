function output=CoreFHorzExpAssetU_CrossTests_nod1_noa1_nosemiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% Markov-as-iid equivalence cross-test, noa1 nosemiz nod1 version.
% Mirrors CoreFHorzExpAssetU_CrossTests_nod1_nosemiz.m but with the experience asset as the only endogenous state.
% n_a is scalar (n_a_justexpasset); a_grid is the a2_grid; n_a_big/a_grid_big unused.
...
ReturnFn_none=@(d2,a,r,w,kappa_j,sigma,agej,Jr,pension) ReturnFn_nod1_noz_noe_noa1_nosemiz(d2,a,r,w,kappa_j,sigma,agej,Jr,pension);
...
ReturnFn_e   =@(d2,a,e,r,w,kappa_j,sigma,agej,Jr,pension) ReturnFn_nod1_noz_e_noa1_nosemiz(d2,a,e,r,w,kappa_j,sigma,agej,Jr,pension);
ReturnFn_ze  =@(d2,a,z,e,r,w,kappa_j,sigma,agej,Jr,pension) ReturnFn_nod1_z_e_noa1_nosemiz(d2,a,z,e,r,w,kappa_j,sigma,agej,Jr,pension);

% Experience asset (common)
vfoptions=struct();
simoptions=struct();
...
%% (1) z as single point with value 1 and prob 1 -> same as no shocks
...
fprintf('Cross test (noa1): z degenerate vs no shocks, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V0(:)-V0z(:))),max(abs(Policy0(:)-Policy0z(:))),max(abs(StationaryDist0(:)-StationaryDist0z(:))))

%% (2) Markov that is just iid in disguise == iid e
...
fprintf('Cross test (noa1): iid-markov-z == iid-e, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V2(:))),max(abs(Policy1(:)-Policy2(:))),max(abs(StationaryDist1(:)-StationaryDist2(:))))
...
fprintf('Cross test (noa1): z+e with z degenerate, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V3(:))),max(abs(Policy1(:)-Policy3(:))),max(abs(StationaryDist1(:)-StationaryDist3(:))))
...
fprintf('Cross test (noa1): z+e with e degenerate, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V4(:))),max(abs(Policy1(:)-Policy4(:))),max(abs(StationaryDist1(:)-StationaryDist4(:))))
