function output=CoreFHorzExpAssetU_CrossTests_nod1_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
...
fprintf('Cross test (noa1+semiz): z degenerate vs no shocks, this should be zero: V %.3e, Policy %.3e, Dist %.3e \n',max(abs(V0(:)-V0z(:))),max(abs(Policy0(:)-Policy0z(:))),max(abs(StationaryDist0(:)-StationaryDist0z(:))))
...
fprintf('Cross test (noa1+semiz): iid-markov-z == iid-e, this should be zero: V %.3e, Policy %.3e, Dist %.3e \n',max(abs(V1(:)-V2(:))),max(abs(Policy1(:)-Policy2(:))),max(abs(StationaryDist1(:)-StationaryDist2(:))))
