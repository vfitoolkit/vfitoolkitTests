function output=CoreFHorzExpAssete_CrossTests3_d1_noz_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% Cross-test 3 (d1, semiz): noa1 ExpAssete+SemiExo vs a1=1 degenerate ExpAssete+SemiExo.
...
% Follows CoreFHorzExpAsset_CrossTests4_d1_semiz, adapted to experienceassete
...
%% semiz, z, e
ReturnFn_A=@(d1,d2,d3,a,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssete_d1_noz_e_noa1_semiz(d1,d2,d3,a,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_B=@(d1,d2,d3,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssete_d1_noz_e_noa1_semiz(d1,d2,d3,a2,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);

jequaloneDist_A=zeros([n_a,n_semiz,vfoptions.n_e],'gpuArray');
jequaloneDist_A(1,ceil(n_semiz/2),ceil(vfoptions.n_e/2))=1;
jequaloneDist_B=zeros([n_a_B,n_semiz,vfoptions.n_e],'gpuArray');
jequaloneDist_B(1,1,ceil(n_semiz/2),ceil(vfoptions.n_e/2))=1;
...
fprintf('Cross test 3 (noa1 vs a1=1, d1 semiz noz e): sim panel means should roughly match: %2.8f vs %2.8f \n',mean(SimPanel_A.assets,'all'),mean(SimPanel_B.assets,'all'))
