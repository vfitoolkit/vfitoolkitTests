function output=CoreFHorzExpAssete_CrossTests2_nod1_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
...
ReturnFn=@(d2,a,e,r,w,kappa_j,sigma,agej,Jr,pension) ReturnFn_ExpAssete_nod1_noz_e_noa1(d2,a,e,r,w,kappa_j,sigma,agej,Jr,pension);
...
fprintf('CrossTest2 (noa1: fake-e-ignored experienceassete vs plain experienceasset), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
