function output=CoreFHorzExpAssete_CrossTests2_d1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 2 for experienceassete+semiz (d1 version): 'fake' experienceassete+semiz whose aprimeFn
% ignores e, vs plain experienceasset+semiz. Both have e and semiz. Should give same V, Policy, StationaryDist.
%
...
jequaloneDist(1,1,ceil(vfoptionsA.n_semiz/2),ceil(vfoptionsA.n_e/2))=1;
...
fprintf('CrossTest2+semiz (fake-e-ignored experienceassete vs plain experienceasset; d1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
