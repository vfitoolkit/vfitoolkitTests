function output=CoreFHorzExpAssete_CrossTests2_nod1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 2 for experienceassete: 'fake' experienceassete whose aprimeFn ignores e, vs plain experienceasset.
% Both models have e as an ordinary iid shock (used in the ReturnFn) -- only the asset-type machinery differs.
...
% 'fake' experienceassete aprimeFn: takes e but ignores it
...
% Model B: plain experienceasset (with same e shock present, but aprimeFn doesn't depend on e)
vfoptionsB=struct();
...
fprintf('CrossTest2 (fake-e-ignored experienceassete vs plain experienceasset), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
