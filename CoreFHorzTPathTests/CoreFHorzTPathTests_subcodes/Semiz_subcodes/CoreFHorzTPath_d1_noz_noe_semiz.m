function output=CoreFHorzTPath_d1_noz_noe_semiz(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
jequaloneDist_big=zeros([n_a_big,vfoptions.n_semiz],'gpuArray'); % Note: based on n_a_big
jequaloneDist_big(1,ceil(vfoptions.n_semiz/2))=1;
jequaloneDist=zeros([n_a,vfoptions.n_semiz],'gpuArray'); % Note: based on n_a
jequaloneDist(1,ceil(vfoptions.n_semiz/2))=1;

ReturnFn=@(d1,d2,aprime,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost) ReturnFn_d1_noz_noe_semiz(d1,d2,aprime,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost);
...
V_final=zeros([n_a,vfoptions.n_semiz,N_j],'gpuArray');
Policy_final=ones([3,n_a,vfoptions.n_semiz,N_j],'gpuArray');
Policy_final_GI=ones([5,n_a,vfoptions.n_semiz,N_j],'gpuArray');
% big versions of them
V_final_big=zeros([n_a_big,vfoptions.n_semiz,N_j],'gpuArray');
Policy_final_big=ones([3,n_a_big,vfoptions.n_semiz,N_j],'gpuArray');
Policy_final_big_GI=ones([5,n_a_big,vfoptions.n_semiz,N_j],'gpuArray');
...
fprintf('Do nothing TPath, this should be zero, AgentDist: %2.8f \n',max(max(max(max(max(abs(AgentDistPath1-repmat(AgentDist1,1,1,1,T))))))))
fprintf('Do nothing TPath, this should be zero, AgentDistB: %2.8f \n',max(max(max(max(max(abs(AgentDistPath1B-repmat(AgentDist1,1,1,1,T))))))))
