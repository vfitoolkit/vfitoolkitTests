function output=CoreFHorzTPath_CrossTests2_nod1_semiz(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline)
% TPath version of CoreFHorz_CrossTests2_nod1_semiz: set up a semi-exo state whose
...
% n_d=n_d2_semiz;  d_grid=d2_grid_semiz;
...
ReturnFn_semiz=@(d2,aprime,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost)...
    ReturnFn_nod1_noz_noe_semiz(d2,aprime,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_semize=@(d2,aprime,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost)...
    ReturnFn_nod1_noz_e_semiz(d2,aprime,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);

ReturnFn_z=@(aprime,a,z,r,w,kappa_j,sigma,agej,Jr,pension)...
    ReturnFn_nod_z_noe_nosemiz(aprime,a,z,r,w,kappa_j,sigma,agej,Jr,pension);
ReturnFn_ze=@(aprime,a,z,e,r,w,kappa_j,sigma,agej,Jr,pension)...
...
Policy_final1A=ones([1,n_a,n_z,N_j],'gpuArray');
[VPath1A,PolicyPath1A]=ValueFnOnTransPath_Case1_FHorz(PricePath, ParamPath, T, V_final1A, Policy_final1A, Params, 0, n_a, n_z, N_j, [], a_grid,z_grid,pi_z, DiscountFactorParamNames, ReturnFn_z, transpathoptionsbaseline, vfoptions1A);
AgentDist_initial1A=StationaryDist_FHorz_Case1(jequaloneDist1,AgeWeightParamNames,PolicyPath1A(:,:,:,:,1),0,n_a,n_z,N_j,pi_z,Params,simoptions1A);
AgentDistPath1A=AgentDistOnTransPath_Case1_FHorz(AgentDist_initial1A, jequaloneDist1, PricePath, ParamPath, PolicyPath1A, AgeWeightParamNames,0,n_a,n_z,N_j,pi_z, T,Params, transpathoptionsbaseline, simoptions1A);
...
Policy_final1B=ones([2,n_a,n_semiz,N_j],'gpuArray');
...
PolicyPath1Bshort=PolicyPath1B(2,:,:,:,:); % remove the d2 policy (not relevant, and not in PolicyPath1A)
...
Policy_final2A=ones([1,n_a,n_z,vfoptions.n_e,N_j],'gpuArray');
[VPath2A,PolicyPath2A]=ValueFnOnTransPath_Case1_FHorz(PricePath, ParamPath, T, V_final2A, Policy_final2A, Params, 0, n_a, n_z, N_j, [], a_grid,z_grid,pi_z, DiscountFactorParamNames, ReturnFn_ze, transpathoptionsbaseline, vfoptions2A);
AgentDist_initial2A=StationaryDist_FHorz_Case1(jequaloneDist1,AgeWeightParamNames,PolicyPath2A(:,:,:,:,:,1),0,n_a,n_z,N_j,pi_z,Params,simoptions2A);
AgentDistPath2A=AgentDistOnTransPath_Case1_FHorz(AgentDist_initial2A, jequaloneDist1, PricePath, ParamPath, PolicyPath2A, AgeWeightParamNames,0,n_a,n_z,N_j,pi_z, T,Params, transpathoptionsbaseline, simoptions2A);
...
Policy_final2B=ones([2,n_a,n_semiz,vfoptions.n_e,N_j],'gpuArray');
...
PolicyPath2Bshort=PolicyPath2B(2,:,:,:,:,:); % remove the d2 policy (not relevant, and not in PolicyPath2A)
