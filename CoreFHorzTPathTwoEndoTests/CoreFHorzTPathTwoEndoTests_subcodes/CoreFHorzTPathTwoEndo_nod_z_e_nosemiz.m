function output=CoreFHorzTPathTwoEndo_nod_z_e_nosemiz(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
ReturnFn=@(a1prime,a2prime,a1,a2,z,e,r,w,kappa_j,sigma,agej,Jr,pension,phi1,phi2) ReturnFn_nod_z_e_nosemiz(a1prime,a2prime,a1,a2,z,e,r,w,kappa_j,sigma,agej,Jr,pension,phi1,phi2);
...
Policy_final=ones([2,n_a,n_z,vfoptions.n_e,N_j],'gpuArray');
Policy_final_GI=ones([4,n_a,n_z,vfoptions.n_e,N_j],'gpuArray');
% big versions of them
V_final_big=zeros([n_a_big,n_z,vfoptions.n_e,N_j],'gpuArray');
Policy_final_big=ones([2,n_a_big,n_z,vfoptions.n_e,N_j],'gpuArray');
Policy_final_big_GI=ones([4,n_a_big,n_z,vfoptions.n_e,N_j],'gpuArray');
...
fprintf('low memory 2 (slowOLG), this should be zero: %.3e \n',max(abs(VPath1(:)-VPath1C(:))))
...
fprintf('low memory 2 (fastOLG), this should be zero: %.3e \n',max(abs(VPath1(:)-VPath1C(:))))
fprintf('low memory 2 (fastOLG), this should be zero: %.3e \n',max(abs(PolicyPath1(:)-PolicyPath1C(:))))
fprintf('low memory 2 (fastOLG), this should be zero: %.3e \n',max(abs(VPath2(:)-VPath2C(:))))
fprintf('low memory 2 (fastOLG), this should be zero: %.3e \n',max(abs(PolicyPath2(:)-PolicyPath2C(:))))
...
fprintf('low memory 2 (with GI, slowOLG), this should be zero: %.3e \n',max(abs(VPath3(:)-VPath3C(:))))
...
fprintf('low memory 2 (with GI, fastOLG), this should be zero: %.3e \n',max(abs(VPath3(:)-VPath3C(:))))
fprintf('low memory 2 (with GI, fastOLG), this should be zero: %.3e \n',max(abs(PolicyPath3(:)-PolicyPath3C(:))))
fprintf('low memory 2 (with GI, fastOLG), this should be zero: %.3e \n',max(abs(VPath4(:)-VPath4C(:))))
fprintf('low memory 2 (with GI, fastOLG), this should be zero: %.3e \n',max(abs(PolicyPath4(:)-PolicyPath4C(:))))
...
[AggVarsPath2.house.Mean; AggVarsPath4.house.Mean]

%% Do some graphs of the AggVars path to see them
fig=figure(figure_c);
subplot(3,1,1); plot(1:1:T,AggVarsPath2.earnings.Mean, 1:1:T,AggVarsPath4.earnings.Mean)
title('Earnings Mean'); legend('1','2')
subplot(3,1,2); plot(1:1:T,AggVarsPath2.assets.Mean, 1:1:T,AggVarsPath4.assets.Mean)
title('Assets Mean'); legend('1','2')
subplot(3,1,3); plot(1:1:T,AggVarsPath2.house.Mean, 1:1:T,AggVarsPath4.house.Mean)
title('House Mean'); legend('1','2')
