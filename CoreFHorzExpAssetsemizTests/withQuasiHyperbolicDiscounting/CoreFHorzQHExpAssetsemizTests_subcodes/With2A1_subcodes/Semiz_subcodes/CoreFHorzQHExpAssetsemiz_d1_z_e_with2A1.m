function output=CoreFHorzQHExpAssetsemiz_d1_z_e_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Quasi-hyperbolic experienceassetsemiz + semiz with TWO standard endogenous assets -> triggers the
...
% a = [a1 (standard), a1_2 (standard), a2 (experience asset)]. n_d=[n_d1,n_d2,n_d3]; d_grid=[d1_grid; d2_grid; d3_grid]
% Note: n_a=[n_a1,n_a1_2,n_a2], a_grid=[a1_grid;a1_2_grid;a2_grid] and Params.r2 are built by
% the calling test script (CoreFHorzExpAssetsemiz_setup), exactly as for the baseline counterpart.
%
...
fprintf('%s lowmemory=3 (DC2A), this should be zero: %.3e \n',qh,max(abs(V2(:)-V2D(:))))
fprintf('%s lowmemory=3 (DC2A, Valt), this should be zero: %.3e \n',qh,max(abs(V2alt(:)-V2Dalt(:))))
fprintf('%s lowmemory=3 (DC2A, Policy), this should be zero: %.3e \n',qh,max(abs(Policy2(:)-Policy2D(:))))
fprintf('%s lowmemory=3 (DC2A, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy2alt(:)-Policy2Dalt(:))));
...
fprintf('%s lowmemory=3 (GI2A), this should be zero: %.3e \n',qh,max(abs(V3(:)-V3D(:))))
fprintf('%s lowmemory=3 (GI2A, Valt), this should be zero: %.3e \n',qh,max(abs(V3alt(:)-V3Dalt(:))))
fprintf('%s lowmemory=3 (GI2A, Policy), this should be zero: %.3e \n',qh,max(abs(Policy3(:)-Policy3D(:))))
fprintf('%s lowmemory=3 (GI2A, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy3alt(:)-Policy3Dalt(:))));
...
fprintf('%s lowmemory=3 (DC2A_GI2A), this should be zero: %.3e \n',qh,max(abs(V4(:)-V4D(:))))
fprintf('%s lowmemory=3 (DC2A_GI2A, Valt), this should be zero: %.3e \n',qh,max(abs(V4alt(:)-V4Dalt(:))))
fprintf('%s lowmemory=3 (DC2A_GI2A, Policy), this should be zero: %.3e \n',qh,max(abs(Policy4(:)-Policy4D(:))))
fprintf('%s lowmemory=3 (DC2A_GI2A, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy4alt(:)-Policy4Dalt(:))));
