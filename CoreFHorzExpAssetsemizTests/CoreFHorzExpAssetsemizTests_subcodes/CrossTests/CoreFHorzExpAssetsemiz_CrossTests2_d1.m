function output=CoreFHorzExpAssetsemiz_CrossTests2_d1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 2 for experienceassetsemiz (with d1): a 'fake' experienceassetsemiz whose
% aprimeFn ignores semiz must match plain experienceasset (with the same semiz present).
% Should give the same V, Policy and StationaryDist.

% n_d passed as [n_d1,n_d2,n_d3].
...
fprintf('CrossTest2 with d1 (fake-semiz-ignored experienceassetsemiz vs plain experienceasset), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
