function output=CoreFHorzExpAssetsemiz_CrossTests_d1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 1 for experienceassetsemiz (with d1): with a degenerate semiz decision
% (n_d3=1), semiz is just an ordinary Markov state, so experienceassetsemiz must
% match experienceassetz with z playing the role of semiz.
...
z_grid_B=semiz_grid;
pi_z_B=pi_semiz;
ReturnFn_B=@(d1,d2,a1prime,a1,a2,z,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit) ReturnFn_ExpAssetsemiz_CrossTest_zside_d1(d1,d2,a1prime,a1,a2,z,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit);
...
fprintf('CrossTest1 with d1 (experienceassetsemiz degenerate-semiz vs experienceassetz), this should be zero: V %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
