function output=CoreFHorzExpAssetsemiz_CrossTests4_d1_noz_e_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% Cross-test 4 (d1, noz, e): noa1 ExpAssetsemiz vs a1=1 degenerate ExpAssetsemiz.
...
% Common semiz + ExpAssetsemiz setup (with e, no z)
vfoptions=struct();
simoptions=struct();
...
%% (semiz, e)
ReturnFn_A=@(d1,d2,d3,a,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetsemiz_d1_noz_e_noa1(d1,d2,d3,a,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_B=@(d1,d2,d3,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetsemiz_d1_noz_e_noa1(d1,d2,d3,a2,semiz,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);

jequaloneDist_A=zeros([n_a,n_semiz,n_e],'gpuArray');
jequaloneDist_A(1,ceil(n_semiz/2),ceil(n_e/2))=1;
jequaloneDist_B=zeros([n_a_B,n_semiz,n_e],'gpuArray');
jequaloneDist_B(1,1,ceil(n_semiz/2),ceil(n_e/2))=1;
...
FnsA.humancapital=@(d1,d2,d3,a,semiz,e) a;
FnsB.humancapital=@(d1,d2,d3,a1prime,a1,a2,semiz,e) a2;
...
fprintf('Cross test 4 (noa1 vs a1=1, d1 semiz noz e): this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f, AllStats.Mean %2.8f, SimPanel.Mean %2.8f \n',max(abs(V_A(:)-V_B(:))),max(abs(Policy_A(:)-PolicyDpart_B(:))),max(abs(StationaryDist_A(:)-StationaryDist_B(:))),abs(AllStats_A.humancapital.Mean-AllStats_B.humancapital.Mean),abs(mean(SimPanel_A.humancapital,'all')-mean(SimPanel_B.humancapital,'all')))
