function output=CoreFHorzExpAssetsemiz_d1_z_e_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% experienceassetsemiz with TWO standard endogenous assets. semiz always present. With ordinary z, with e.
...
% zeros assets, mid points for semiz, z and e (bothz ordering: [semiz,z], then e)
...
ReturnFn=@(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e,r,r2,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetsemiz_d1_z_e_with2A1(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e,r,r2,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);

% Setup some FnsToEvaluate
FnsToEvaluate.assets=@(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e) a1;
FnsToEvaluate.assets2=@(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e) a1_2;
FnsToEvaluate.humancapital=@(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e) a2;
FnsToEvaluate.earnings=@(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e,w,kappa_j) w*kappa_j*d1*d2*a2*semiz*z*e;
...
fprintf('Divide-and-conquer (DC2A), this should be zero: %.3e \n',max(abs(V1(:)-V2(:))))
...
fprintf('lowmemory=1 (with DC2A), this should be zero: %.3e \n',max(abs(V2(:)-V2B(:))))
...
fprintf('lowmemory=2 (with DC2A), this should be zero: %.3e \n',max(abs(V2(:)-V2C(:))))
...
fprintf('lowmemory=3 (with DC2A), this should be zero: %.3e \n',max(abs(V2(:)-V2D(:))))
fprintf('lowmemory=3 (with DC2A), this should be zero: %.3e \n',max(abs(Policy2(:)-Policy2D(:))))
vfoptions2.lowmemory=0;

%%
clear V1 V2 V1B V1C V1D V2B V2C V2D Policy1 Policy2 Policy1B Policy1C Policy1D Policy2B Policy2C Policy2D PolicyVals1 V1fromPolicy
...
fprintf('Divide-and-conquer (with Grid Interp Layer, DC2A_GI2A), this should be zero: %.3e \n',max(abs(V3(:)-V4(:))))
...
fprintf('lowmemory=1 (with GI2A), this should be zero: %.3e \n',max(abs(V3(:)-V3B(:))))
...
fprintf('lowmemory=2 (with GI2A), this should be zero: %.3e \n',max(abs(V3(:)-V3C(:))))
...
fprintf('lowmemory=3 (with GI2A), this should be zero: %.3e \n',max(abs(V3(:)-V3D(:))))
...
fprintf('lowmemory=1  (with DC2A+GI2A), this should be zero: %.3e \n',max(abs(V4(:)-V4B(:))))
...
fprintf('lowmemory=2  (with DC2A+GI2A), this should be zero: %.3e \n',max(abs(V4(:)-V4C(:))))
...
fprintf('lowmemory=3  (with DC2A+GI2A), this should be zero: %.3e \n',max(abs(V4(:)-V4D(:))))
fprintf('lowmemory=3  (with DC2A+GI2A), this should be zero: %.3e \n',max(abs(Policy4(:)-Policy4D(:))))
vfoptions4.lowmemory=0;


%%
clear V3 V4 V3B V3C V3D V4B V4C V4D Policy3 Policy4 Policy3B Policy3C Policy3D Policy4B Policy4C Policy4D PolicyVals3 V3fromPolicy
...
fprintf('Sim panel means should roughly match the age-conditional stats (plain) \n')
...
fprintf('Sim panel means should roughly match the age-conditional stats (grid interp) \n')
...
title('Earnings Mean (experienceassetsemiz, with2A1, d1, z, e)')
legend('1','3')
subplot(3,1,2); plot(1:1:N_j,AgeConditionalStats1.assets2.Mean, 1:1:N_j,AgeConditionalStats3.assets2.Mean)
title('Second standard asset (binary) Mean')
legend('1','3')
