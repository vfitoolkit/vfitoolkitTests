function output=CoreInfHorz_CrossTests_nod_nosemiz_2Aignored(n_d,n_a,n_a_big,n_z,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,vfoptionsbaseline,simoptionsbaseline)
...
% it is economically the same problem as the one endogenous state model (which is the fig 3 model).
% Hence V and the a1prime policy must be identical (with a2 just replicating them).
...
ReturnFn_2A=@(aprime,a2prime,a,a2,z,r,w,sigma) ReturnFn_nod_z_noe_nosemiz_2Aignored(aprime,a2prime,a,a2,z,r,w,sigma);
FnsToEvaluate_2A.assets=@(aprime,a2prime,a,a2,z) a;

fprintf('\n================ CROSS-TEST: 2A with ignored second asset vs 1A (nod) ================\n');
...
for gi=0:1
    ...
    fprintf('%s: V, 2A(ignored a2) vs 1A, should be zero:            %.3e \n',gistr,max(abs(reshape(V2A,[n_a,n_a2,N_z])-V1Arep),[],'all'));

    % Policy: compare the aprime VALUES (a2prime is an arbitrary tie-break, so is not compared)
    ...
    aprime1A=repmat(reshape(PolicyVals1A(1,:,:),[n_a,1,N_z]),1,n_a2,1);
    aprime2A=reshape(PolicyVals2A(1,:,:,:),[n_a,n_a2,N_z]);
    fprintf('%s: aprime policy values, 2A vs 1A, should be zero:     %.3e \n',gistr,max(abs(aprime2A-aprime1A),[],'all'));
    ...
    fprintf('%s: ValueFnFromPolicy on 2A, should be zero:            %.3e \n',gistr,max(abs(V2AfromPolicy(:)-V2A(:))));
    ...
    fprintf('%s: AllStats assets Mean, 2A vs 1A, should be zero:     %.3e \n',gistr,abs(AllStats2A.assets.Mean-AllStats1A.assets.Mean));
    fprintf('%s: AllStats assets Gini, 2A vs 1A, should be zero:     %.3e \n',gistr,abs(AllStats2A.assets.Gini-AllStats1A.assets.Gini));
end
