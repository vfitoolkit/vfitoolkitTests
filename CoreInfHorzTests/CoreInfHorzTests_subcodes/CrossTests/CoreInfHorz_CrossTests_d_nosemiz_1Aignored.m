function output=CoreInfHorz_CrossTests_d_nosemiz_1Aignored(n_d,n_a,n_a_big,n_z,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,vfoptionsbaseline,simoptionsbaseline)
% CROSS-TEST: two endogenous states where the FIRST one does nothing, vs just one endogenous state.
% With d.
%
% This is the mirror image of CoreInfHorz_CrossTests_d_nosemiz_2Aignored: here it is a1 that the
% return function ignores, and a2 that is the actual asset. So it is again economically the same
% problem as the one endogenous state model (which is the fig 4 model), and V, the d policy and the
% a2prime policy must be identical (with a1 just replicating them).
%
...
ReturnFn_2A=@(d,a1prime,a2prime,a1,a2,z,r,w,sigma,eta,varphi) ReturnFn_d_z_noe_nosemiz_1Aignored(d,a1prime,a2prime,a1,a2,z,r,w,sigma,eta,varphi);
FnsToEvaluate_2A.assets=@(d,a1prime,a2prime,a1,a2,z) a2;

fprintf('\n================ CROSS-TEST: 2A with ignored FIRST asset vs 1A (with d) ================\n');
...
% Policy: compare the d and a2prime VALUES (a1prime is an arbitrary tie-break, so is not compared)
...
d1A=repmat(reshape(PolicyVals1A(1,:,:),[1,n_a,N_z]),n_a1,1,1);
d2A=reshape(PolicyVals2A(1,:,:,:),[n_a1,n_a,N_z]);
fprintf('no GI : d policy values, 2A vs 1A, should be zero:          %2.8f \n',max(abs(d2A-d1A),[],'all'));
a2prime1A=repmat(reshape(PolicyVals1A(2,:,:),[1,n_a,N_z]),n_a1,1,1);
a2prime2A=reshape(PolicyVals2A(3,:,:,:),[n_a1,n_a,N_z]);
