function output=QHDFHorzTPath_nod_noz_noe_nosemiz(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
jequaloneDist_big=zeros(n_a_big,1,'gpuArray'); % Note: based on n_a_big
jequaloneDist_big(1)=1;
jequaloneDist=zeros(n_a,1,'gpuArray'); % Note: based n_a
jequaloneDist(1)=1;

ReturnFn=@(aprime,a,r,w,kappa_j,sigma,agej,Jr,pension) ReturnFn_nod_noz_noe_nosemiz(aprime,a,r,w,kappa_j,sigma,agej,Jr,pension);
...
fprintf('fastOLG (Naive), this should be zero: %2.8f \n',max(abs(VPath1slow(:)-VPath1fast(:))))
fprintf('fastOLG (Naive), this should be zero: %2.8f \n',max(abs(PolicyPath1slow(:)-PolicyPath1fast(:))))
fprintf('fastOLG (Naive, Valt), this should be zero: %2.8f \n',max(abs(ValtPath1slow(:)-ValtPath1fast(:))))
fprintf('fastOLG (Naive, Policyalt), this should be zero: %2.8f \n',max(abs(PolicyaltPath1slow(:)-PolicyaltPath1fast(:))))
...
fprintf('Divide-and-conquer (slowOLG, Naive), this should be zero: %2.8f \n',max(abs(VPath1(:)-VPath2(:))))
fprintf('Divide-and-conquer (slowOLG, Naive), this should be zero: %2.8f \n',max(abs(PolicyPath1(:)-PolicyPath2(:))))
fprintf('Divide-and-conquer (slowOLG, Naive, Valt), this should be zero: %2.8f \n',max(abs(ValtPath1(:)-ValtPath2(:))))
fprintf('Divide-and-conquer (slowOLG, Naive, Policyalt), this should be zero: %2.8f \n',max(abs(PolicyaltPath1(:)-PolicyaltPath2(:))))
...
fprintf('Divide-and-conquer (fastOLG, Naive), this should be zero: %2.8f \n',max(abs(VPath1(:)-VPath2(:))))
fprintf('Divide-and-conquer (fastOLG, Naive), this should be zero: %2.8f \n',max(abs(PolicyPath1(:)-PolicyPath2(:))))
fprintf('Divide-and-conquer (fastOLG, Naive, Valt), this should be zero: %2.8f \n',max(abs(ValtPath1(:)-ValtPath2(:))))
fprintf('Divide-and-conquer (fastOLG, Naive, Policyalt), this should be zero: %2.8f \n',max(abs(PolicyaltPath1(:)-PolicyaltPath2(:))))
...
fprintf('Divide-and-conquer (with GI, slowOLG, Naive), this should be zero: %2.8f \n',max(abs(VPath3(:)-VPath4(:))))
fprintf('Divide-and-conquer (with GI, slowOLG, Naive), this should be zero: %2.8f \n',max(abs(PolicyPath3(:)-PolicyPath4(:))))
fprintf('Divide-and-conquer (with GI, slowOLG, Naive, Valt), this should be zero: %2.8f \n',max(abs(ValtPath3(:)-ValtPath4(:))))
...
fprintf('Divide-and-conquer (with GI, fastOLG, Naive), this should be zero: %2.8f \n',max(abs(VPath3(:)-VPath4(:))))
fprintf('Divide-and-conquer (with GI, fastOLG, Naive), this should be zero: %2.8f \n',max(abs(PolicyPath3(:)-PolicyPath4(:))))
fprintf('Divide-and-conquer (with GI, fastOLG, Naive, Valt), this should be zero: %2.8f \n',max(abs(ValtPath3(:)-ValtPath4(:))))
...
fprintf('Do nothing TPath (Naive), this should be zero, V: %2.8f \n',max(abs(VPath1(:)-reshape(repmat(V1,1,1,T),[],1))))
fprintf('Do nothing TPath (Naive), this should be zero, Policy: %2.8f \n',max(abs(PolicyPath1(:)-reshape(repmat(Policy1,1,1,1,T),[],1))))
fprintf('Do nothing TPath (Naive), this should be zero, Valt: %2.8f \n',max(abs(ValtPath1(:)-reshape(repmat(V1alt,1,1,T),[],1))))
fprintf('Do nothing TPath (Naive), this should be zero, Policyalt: %2.8f \n',max(abs(PolicyaltPath1(:)-reshape(repmat(Policy1alt,1,1,1,T),[],1))))
...
fprintf('One iter of TPath (Naive), with/without fastOLG, this should be zero: %2.8f \n',max(abs(PricePath2.r-PricePath2B.r)))
...
fprintf('One iter of TPath (Naive), with/without GI, this should be close to zero: %2.8f \n',max(abs(PricePath3A.r-PricePath3B.r)))
...
fprintf('fastOLG (Sophisticated), this should be zero: %2.8f \n',max(abs(VPath1slow(:)-VPath1fast(:))))
fprintf('fastOLG (Sophisticated), this should be zero: %2.8f \n',max(abs(PolicyPath1slow(:)-PolicyPath1fast(:))))
fprintf('fastOLG (Sophisticated, Vunderbar), this should be zero: %2.8f \n',max(abs(ValtPath1slow(:)-ValtPath1fast(:))))
...
fprintf('Divide-and-conquer (slowOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(VPath1(:)-VPath2(:))))
fprintf('Divide-and-conquer (slowOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(PolicyPath1(:)-PolicyPath2(:))))
fprintf('Divide-and-conquer (slowOLG, Sophisticated, Vunderbar), this should be zero: %2.8f \n',max(abs(ValtPath1(:)-ValtPath2(:))))
...
fprintf('Divide-and-conquer (fastOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(VPath1(:)-VPath2(:))))
fprintf('Divide-and-conquer (fastOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(PolicyPath1(:)-PolicyPath2(:))))
fprintf('Divide-and-conquer (fastOLG, Sophisticated, Vunderbar), this should be zero: %2.8f \n',max(abs(ValtPath1(:)-ValtPath2(:))))
...
fprintf('Divide-and-conquer (with GI, slowOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(VPath3(:)-VPath4(:))))
fprintf('Divide-and-conquer (with GI, slowOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(PolicyPath3(:)-PolicyPath4(:))))
fprintf('Divide-and-conquer (with GI, slowOLG, Sophisticated, Vunderbar), this should be zero: %2.8f \n',max(abs(ValtPath3(:)-ValtPath4(:))))
...
fprintf('Divide-and-conquer (with GI, fastOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(VPath3(:)-VPath4(:))))
fprintf('Divide-and-conquer (with GI, fastOLG, Sophisticated), this should be zero: %2.8f \n',max(abs(PolicyPath3(:)-PolicyPath4(:))))
fprintf('Divide-and-conquer (with GI, fastOLG, Sophisticated, Vunderbar), this should be zero: %2.8f \n',max(abs(ValtPath3(:)-ValtPath4(:))))
...
fprintf('Do nothing TPath (Sophisticated), this should be zero, V: %2.8f \n',max(abs(VPath1(:)-reshape(repmat(V1,1,1,T),[],1))))
fprintf('Do nothing TPath (Sophisticated), this should be zero, Policy: %2.8f \n',max(abs(PolicyPath1(:)-reshape(repmat(Policy1,1,1,1,T),[],1))))
fprintf('Do nothing TPath (Sophisticated), this should be zero, Vunderbar: %2.8f \n',max(abs(ValtPath1(:)-reshape(repmat(V1under,1,1,T),[],1))))
...
fprintf('One iter of TPath (Sophisticated), with/without fastOLG, this should be zero: %2.8f \n',max(abs(PricePath2.r-PricePath2B.r)))
...
fprintf('One iter of TPath (Sophisticated), with/without GI, this should be close to zero: %2.8f \n',max(abs(PricePath3A.r-PricePath3B.r)))
...
fprintf('QH-TPath with beta0=1 (Naive vs Exp), this should be zero: V    %2.8f \n',max(abs(VPathA(:)-VPathB(:))))
fprintf('QH-TPath with beta0=1 (Naive vs Exp), this should be zero: P    %2.8f \n',max(abs(PolicyPathA(:)-PolicyPathB(:))))
fprintf('QH-TPath with beta0=1 (Naive vs Exp, Valt), this should be zero: %2.8f \n',max(abs(VPathA(:)-ValtPathB(:))))
fprintf('QH-TPath with beta0=1 (Soph vs Exp), this should be zero:  V    %2.8f \n',max(abs(VPathA(:)-VPathC(:))))
fprintf('QH-TPath with beta0=1 (Soph vs Exp), this should be zero:  P    %2.8f \n',max(abs(PolicyPathA(:)-PolicyPathC(:))))
fprintf('QH-TPath with beta0=1 (Soph vs Exp, Vunderbar), this should be zero: %2.8f \n',max(abs(VPathA(:)-ValtPathC(:))))
