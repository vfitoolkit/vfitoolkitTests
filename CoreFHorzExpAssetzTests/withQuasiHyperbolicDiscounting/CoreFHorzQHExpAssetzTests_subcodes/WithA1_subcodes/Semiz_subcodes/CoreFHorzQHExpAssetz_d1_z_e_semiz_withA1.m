function output=CoreFHorzQHExpAssetz_d1_z_e_semiz_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Quasi-hyperbolic experienceassetz WITH semi-exogenous state, single standard endogenous asset a1.
% Full method sweep: base / DC1 / GI1 / DC1_GI1 (single standard asset a1 -> DC1 not DC2A), for Naive
% then Sophisticated, with a ValueFnFromPolicy oracle on every method and a continuation-value (Valt)
% check beside every V check, then the exponential-discounting cross-tests. GI changes the solution
% slightly vs base, so GI1 is not compared to base for equality (only its own ValueFnFromPolicy and
% DC1_GI1-vs-GI1 are checked).
...
ReturnFn=@(d1,d2,d3,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetz_d1_z_e_semiz(d1,d2,d3,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
