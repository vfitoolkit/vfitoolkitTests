function output=CoreFHorzExpAssetz_d1_z_e_nosemiz_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Tests the DC2A / GI2A / DC2A_GI2A code paths, which are triggered by having TWO
% standard endogenous states (length(n_a1)>1). The dispatcher divide-conquers the
% first standard endogenous state and FOLDS (brute-forces) the remaining ones.
% Here the second standard endogenous state a1_2 is BINARY: a capped high-return
% asset (return r2>r, holdings in {0,1}). The experienceassetz a2 is unchanged.
% a = [a1_1 (liquid, divide-conquered), a1_2 (multi-point, folded), a2 (experience asset)]
...
jequaloneDist(1,1,1,ceil(n_z/2),ceil(vfoptions.n_e/2))=1;

ReturnFn=@(d1,d2,a1prime,a1_2prime,a1,a1_2,a2,z,e,r,r2,w,kappa_j,sigma,varphi,eta,agej,Jr,pension) ReturnFn_ExpAssetz_d1_z_e_with2A1(d1,d2,a1prime,a1_2prime,a1,a1_2,a2,z,e,r,r2,w,kappa_j,sigma,varphi,eta,agej,Jr,pension);

% Setup some FnsToEvaluate
FnsToEvaluate.assets=@(d1,d2,a1prime,a1_2prime,a1,a1_2,a2,z,e) a1;
FnsToEvaluate.assets2=@(d1,d2,a1prime,a1_2prime,a1,a1_2,a2,z,e) a1_2;
FnsToEvaluate.humancapital=@(d1,d2,a1prime,a1_2prime,a1,a1_2,a2,z,e) a2;
FnsToEvaluate.earnings=@(d1,d2,a1prime,a1_2prime,a1,a1_2,a2,z,e,w,kappa_j) w*kappa_j*d1*d2*a2*z*e;
...
[AllStats2.assets2.Mean,AllStats4.assets2.Mean]
...
jequaloneDist5(1,1,1,ceil(n_z/2),ceil(vfoptions.n_e/2))=1;
