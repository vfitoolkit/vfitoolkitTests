function output=CoreFHorzExpAssetz_CrossTests_nod1_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 1 for experienceassetz: compare experienceassetz (with z that is iid-markov;
% i.e., transition matrix has all rows identical) to experienceassete (with normal iid e).
...
fprintf('CrossTest1 (experienceassetz iid-markov-z vs experienceassete iid-e), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
