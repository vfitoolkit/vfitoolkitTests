function output=CoreFHorzExpAssetz_CrossTests3_d1_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% Cross-test 3 (d1, semiz): noa1 ExpAssetz+SemiExo vs a1=1 degenerate ExpAssetz+SemiExo.
...
% z is always present for ExpAssetz; following CoreFHorzExpAsset_CrossTests4 this is
% minimal, so just the shock-richest variant (z + e).
% PENDING TOOLKIT SUPPORT (test-first): the ExpAssetzSemiExo noa1 raws do not exist
% yet, so Side A errors at the ValueFnIter call.
%
...
% Common semiz + ExpAssetz setup (with e)
vfoptions=struct();
simoptions=struct();
...
%% (z, e)
ReturnFn_A=@(d1,d2,d3,a,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetz_d1_z_e_semiz_noa1(d1,d2,d3,a,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_B=@(d1,d2,d3,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetz_d1_z_e_semiz_noa1(d1,d2,d3,a2,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
...
fprintf('Cross test 3 (noa1 vs a1=1, d1 semiz z e): sim panel means should roughly match: %2.8f vs %2.8f \n',mean(SimPanel_A.assets,'all'),mean(SimPanel_B.assets,'all'))
