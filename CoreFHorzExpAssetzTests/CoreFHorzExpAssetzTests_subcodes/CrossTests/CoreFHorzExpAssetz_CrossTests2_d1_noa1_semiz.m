function output=CoreFHorzExpAssetz_CrossTests2_d1_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 2 for experienceassetz+semiz (d1, noa1 version): 'fake' experienceassetz whose
% aprimeFn ignores z, vs plain experienceasset (both with semiz, both with z present in the model).
...
fprintf('CrossTest2+semiz (noa1: fake-z-ignored experienceassetz vs plain experienceasset; d1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
