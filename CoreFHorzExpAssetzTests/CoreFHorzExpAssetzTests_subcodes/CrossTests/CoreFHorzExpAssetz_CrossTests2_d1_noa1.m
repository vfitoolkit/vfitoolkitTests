function output=CoreFHorzExpAssetz_CrossTests2_d1_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 2 for experienceassetz (d1, noa1 version): 'fake' experienceassetz whose aprimeFn
% ignores z, vs plain experienceasset. The experience asset a2 is the only endogenous state on
% both sides. Both models have z as an ordinary Markov shock (used in the ReturnFn) -- only the
% asset-type machinery differs. Should give same V, Policy, StationaryDist.
%
...
ReturnFn=@(d1,d2,a,z,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension) ReturnFn_ExpAssetz_d1_z_noe_noa1(d1,d2,a,z,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension);
...
fprintf('CrossTest2 (noa1: fake-z-ignored experienceassetz vs plain experienceasset; d1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
