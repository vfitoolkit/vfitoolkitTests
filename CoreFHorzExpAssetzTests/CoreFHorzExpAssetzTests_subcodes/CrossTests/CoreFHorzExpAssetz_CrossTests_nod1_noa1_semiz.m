function output=CoreFHorzExpAssetz_CrossTests_nod1_noa1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 1 for experienceassetz+semiz, noa1 version: compare experienceassetz+semiz (with z
% that is iid-markov; transition matrix has all rows identical) to experienceassete+semiz (with
% normal iid e). The experience asset a2 is the only endogenous state on both sides.
...
ReturnFn_zside=@(d2,d3,a,semiz,z,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetz_nod1_z_noe_semiz_noa1(d2,d3,a,semiz,z,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_eside=@(d2,d3,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetz_nod1_z_noe_semiz_noa1(d2,d3,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
...
jequaloneDist_B(1,ceil(vfoptionsB.n_semiz/2),ceil(vfoptionsB.n_e/2))=1;
...
fprintf('CrossTest1+semiz (noa1: experienceassetz iid-markov-z vs experienceassete iid-e; nod1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
