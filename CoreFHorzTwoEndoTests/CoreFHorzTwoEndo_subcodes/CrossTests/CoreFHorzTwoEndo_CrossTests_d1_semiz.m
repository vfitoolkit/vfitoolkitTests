...
% Test 6: with-d1 model where d1 takes 3 values but the ReturnFn ignores it.
% V from the with-d1 code path must equal V from the nod1 code path.

%% Test 6: d1 ignored in ReturnFn -> with-d1 path equals nod1 path
n_d1_t6=3;
...
fprintf('Test 6 (d1 ignored vs nod1), V should be zero: %2.8f \n',max(abs(V_d1(:)-V_nod1(:))))

% Also check with divide-and-conquer
vfopts_dc=vfopts; vfopts_dc.divideandconquer=1;
[V_d1_dc,~]=ValueFnIter_Case1_FHorz(n_d_t6,n_a,0,N_j,d_grid_t6,a_grid,[],[],ReturnFn_d1,Params,DiscountFactorParamNames,[],vfopts_dc);
[V_nod1_dc,~]=ValueFnIter_Case1_FHorz(n_d2_semiz,n_a,0,N_j,d2_grid_semiz,a_grid,[],[],ReturnFn_nod1,Params,DiscountFactorParamNames,[],vfopts_dc);
fprintf('Test 6 (d1 ignored vs nod1, divide-and-conquer), V should be zero: %2.8f \n',max(abs(V_d1_dc(:)-V_nod1_dc(:))))

% Also check with grid interpolation layer
vfopts_gi=vfopts; vfopts_gi.gridinterplayer=1; vfopts_gi.ngridinterp=5;
[V_d1_gi,~]=ValueFnIter_Case1_FHorz(n_d_t6,n_a,0,N_j,d_grid_t6,a_grid,[],[],ReturnFn_d1,Params,DiscountFactorParamNames,[],vfopts_gi);
[V_nod1_gi,~]=ValueFnIter_Case1_FHorz(n_d2_semiz,n_a,0,N_j,d2_grid_semiz,a_grid,[],[],ReturnFn_nod1,Params,DiscountFactorParamNames,[],vfopts_gi);
fprintf('Test 6 (d1 ignored vs nod1, GI layer), V should be zero: %2.8f \n',max(abs(V_d1_gi(:)-V_nod1_gi(:))))

% And with DC + GI together
vfopts_dcgi=vfopts; vfopts_dcgi.divideandconquer=1; vfopts_dcgi.gridinterplayer=1; vfopts_dcgi.ngridinterp=5;
[V_d1_dcgi,~]=ValueFnIter_Case1_FHorz(n_d_t6,n_a,0,N_j,d_grid_t6,a_grid,[],[],ReturnFn_d1,Params,DiscountFactorParamNames,[],vfopts_dcgi);
[V_nod1_dcgi,~]=ValueFnIter_Case1_FHorz(n_d2_semiz,n_a,0,N_j,d2_grid_semiz,a_grid,[],[],ReturnFn_nod1,Params,DiscountFactorParamNames,[],vfopts_dcgi);
fprintf('Test 6 (d1 ignored vs nod1, DC+GI), V should be zero: %2.8f \n',max(abs(V_d1_dcgi(:)-V_nod1_dcgi(:))))
