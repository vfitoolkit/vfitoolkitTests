function output=CoreFHorzQHExpAssetze_d1_z_e_semiz_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Quasi-hyperbolic experienceassetze + semiz, noa1: the experience asset a2 is the only endogenous state.
...
%
% Only the base method is tested: with no a1 there is nothing for DC/GI to operate on
% (same reason the baseline noa1 subcodes have no DC/GI/DC+GI blocks).
% shocks: {semiz (semi-exogenous), z (markov), e (iid)} -> valid lowmemory {0,1,2,3}.
%
% PENDING TOOLKIT SUPPORT: the QH ExpAssetze noa1 raws do not yet exist, so this errors at the
% first ValueFnIter call -- ValueFnIter_FHorz_QuasiHyperbolicExpAssetzeSemiExo.m: 'experienceassetze+semiz
% requires a standard asset a1' (test-first: written ahead of the toolkit code).
...
ReturnFn=@(d1,d2,d3,a2,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetze_d1_z_e_semiz_noa1(d1,d2,d3,a2,semiz,z,e,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
...
fprintf('Naive lowmemory=2, this should be zero: %2.8f \n',max(abs(V1(:)-V1C(:))))
fprintf('Naive lowmemory=2 (Valt), this should be zero: %2.8f \n',max(abs(V1alt(:)-V1Calt(:))))
fprintf('Naive lowmemory=2 (Policy), this should be zero: %2.8f \n',max(abs(Policy1(:)-Policy1C(:))))
...
fprintf('Naive lowmemory=3, this should be zero: %2.8f \n',max(abs(V1(:)-V1D(:))))
fprintf('Naive lowmemory=3 (Valt), this should be zero: %2.8f \n',max(abs(V1alt(:)-V1Dalt(:))))
fprintf('Naive lowmemory=3 (Policy), this should be zero: %2.8f \n',max(abs(Policy1(:)-Policy1D(:))))
...
fprintf('Sophisticated lowmemory=2, this should be zero: %2.8f \n',max(abs(V1s(:)-V1sC(:))))
fprintf('Sophisticated lowmemory=2 (Valt), this should be zero: %2.8f \n',max(abs(V1salt(:)-V1sCalt(:))))
fprintf('Sophisticated lowmemory=2 (Policy), this should be zero: %2.8f \n',max(abs(Policy1s(:)-Policy1sC(:))))
vfoptions1.lowmemory=3;
[V1sD,Policy1sD,V1sDalt]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,[],vfoptions1);
fprintf('Sophisticated lowmemory=3, this should be zero: %2.8f \n',max(abs(V1s(:)-V1sD(:))))
fprintf('Sophisticated lowmemory=3 (Valt), this should be zero: %2.8f \n',max(abs(V1salt(:)-V1sDalt(:))))
fprintf('Sophisticated lowmemory=3 (Policy), this should be zero: %2.8f \n',max(abs(Policy1s(:)-Policy1sD(:))))
vfoptions1.lowmemory=0;
