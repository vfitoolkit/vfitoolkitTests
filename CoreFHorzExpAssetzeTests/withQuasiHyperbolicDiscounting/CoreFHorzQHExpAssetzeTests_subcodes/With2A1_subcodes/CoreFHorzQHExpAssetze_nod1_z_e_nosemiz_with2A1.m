function output=CoreFHorzQHExpAssetze_nod1_z_e_nosemiz_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
fprintf('%s lowmemory=1, this should be zero: %.3e \n',qh,max(abs(V1(:)-V1B(:))))
fprintf('%s lowmemory=1 (Valt), this should be zero: %.3e \n',qh,max(abs(V1alt(:)-V1Balt(:))))
fprintf('%s lowmemory=1 (Policy), this should be zero: %.3e \n',qh,max(abs(Policy1(:)-Policy1B(:))))
fprintf('%s lowmemory=1 (Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy1alt(:)-Policy1Balt(:))));
...
fprintf('%s lowmemory=2, this should be zero: %.3e \n',qh,max(abs(V1(:)-V1C(:))))
fprintf('%s lowmemory=2 (Valt), this should be zero: %.3e \n',qh,max(abs(V1alt(:)-V1Calt(:))))
fprintf('%s lowmemory=2 (Policy), this should be zero: %.3e \n',qh,max(abs(Policy1(:)-Policy1C(:))))
fprintf('%s lowmemory=2 (Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy1alt(:)-Policy1Calt(:))));
