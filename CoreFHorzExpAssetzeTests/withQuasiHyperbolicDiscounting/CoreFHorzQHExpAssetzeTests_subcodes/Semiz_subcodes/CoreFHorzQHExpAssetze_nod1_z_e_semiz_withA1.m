function output=CoreFHorzQHExpAssetze_nod1_z_e_semiz_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Quasi-hyperbolic experienceassetze WITH semi-exogenous state, single standard endogenous asset a1, NO d1.
% Full method sweep: base / DC1 / GI1 / DC1_GI1 (single standard asset a1 -> DC1 not DC2A), for Naive
% then Sophisticated, with a ValueFnFromPolicy oracle on every method, then the exponential-discounting
% cross-tests. GI changes the solution slightly vs base, so GI1 is not compared to base for equality
% (only its own ValueFnFromPolicy + DC1_GI1-vs-GI1 are checked).
% shocks: {semiz, z (markov), e (iid)}.
...
% SemiExo ValueFnFromPolicy oracle on the base method: validates ValueFnFromPolicy_FHorz_QuasiHyperbolic_ExpAssetze_SemiExo
% (the SemiExo grids are already on vfoptions1, so ValueFnFromPolicy_FHorz routes QH->SemiExo->ExpAssetze_SemiExo)
...
fprintf('%s lowmemory=3 (base), this should be zero: %.3e \n',qh,max(abs(V1(:)-V1D(:))))
fprintf('%s lowmemory=3 (base, Valt), this should be zero: %.3e \n',qh,max(abs(V1alt(:)-V1Dalt(:))))
fprintf('%s lowmemory=3 (base, Policy), this should be zero: %.3e \n',qh,max(abs(Policy1(:)-Policy1D(:))))
fprintf('%s lowmemory=3 (base, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy1alt(:)-Policy1Dalt(:))));
vfoptions1.lowmemory=0;

% Divide-and-conquer -> DC1, should give same answer as base
...
fprintf('%s DC1, this should be zero: %.3e \n',qh,max(abs(V1(:)-V2(:))))
fprintf('%s DC1 (Valt), this should be zero: %.3e \n',qh,max(abs(V1alt(:)-V2alt(:))))
fprintf('%s DC1 (Policy), this should be zero: %.3e \n',qh,max(abs(Policy1(:)-Policy2(:))))
fprintf('%s DC1 (Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy1alt(:)-Policy2alt(:))));
% ValueFnFromPolicy oracle on DC1 (non-GI SemiExo VFP)
...
fprintf('%s ValueFnFromPolicy (DC1), this should be zero: %.3e \n',qh,max(abs(V2fromPolicy(:)-V2(:))))
fprintf('%s ValueFnFromPolicy (DC1, Valt), this should be zero: %.3e \n',qh,max(abs(V2altfromPolicy(:)-V2alt(:))))
...
fprintf('%s lowmemory=1 (DC1), this should be zero: %.3e \n',qh,max(abs(V2(:)-V2B(:))))
fprintf('%s lowmemory=1 (DC1, Valt), this should be zero: %.3e \n',qh,max(abs(V2alt(:)-V2Balt(:))))
fprintf('%s lowmemory=1 (DC1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy2(:)-Policy2B(:))))
fprintf('%s lowmemory=1 (DC1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy2alt(:)-Policy2Balt(:))));
...
fprintf('%s lowmemory=2 (DC1), this should be zero: %.3e \n',qh,max(abs(V2(:)-V2C(:))))
fprintf('%s lowmemory=2 (DC1, Valt), this should be zero: %.3e \n',qh,max(abs(V2alt(:)-V2Calt(:))))
fprintf('%s lowmemory=2 (DC1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy2(:)-Policy2C(:))))
fprintf('%s lowmemory=2 (DC1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy2alt(:)-Policy2Calt(:))));
...
fprintf('%s lowmemory=3 (DC1), this should be zero: %.3e \n',qh,max(abs(V2(:)-V2D(:))))
fprintf('%s lowmemory=3 (DC1, Valt), this should be zero: %.3e \n',qh,max(abs(V2alt(:)-V2Dalt(:))))
fprintf('%s lowmemory=3 (DC1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy2(:)-Policy2D(:))))
fprintf('%s lowmemory=3 (DC1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy2alt(:)-Policy2Dalt(:))));
...
% ValueFnFromPolicy oracle on GI1 (routes to the SemiExo-GI VFP via vfoptions.gridinterplayer)
...
fprintf('%s ValueFnFromPolicy (GI1), this should be zero: %.3e \n',qh,max(abs(V3fromPolicy(:)-V3(:))))
fprintf('%s ValueFnFromPolicy (GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V3altfromPolicy(:)-V3alt(:))))
...
fprintf('%s lowmemory=1 (GI1), this should be zero: %.3e \n',qh,max(abs(V3(:)-V3B(:))))
fprintf('%s lowmemory=1 (GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V3alt(:)-V3Balt(:))))
fprintf('%s lowmemory=1 (GI1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy3(:)-Policy3B(:))))
fprintf('%s lowmemory=1 (GI1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy3alt(:)-Policy3Balt(:))));
...
fprintf('%s lowmemory=2 (GI1), this should be zero: %.3e \n',qh,max(abs(V3(:)-V3C(:))))
fprintf('%s lowmemory=2 (GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V3alt(:)-V3Calt(:))))
fprintf('%s lowmemory=2 (GI1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy3(:)-Policy3C(:))))
fprintf('%s lowmemory=2 (GI1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy3alt(:)-Policy3Calt(:))));
...
fprintf('%s lowmemory=3 (GI1), this should be zero: %.3e \n',qh,max(abs(V3(:)-V3D(:))))
fprintf('%s lowmemory=3 (GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V3alt(:)-V3Dalt(:))))
fprintf('%s lowmemory=3 (GI1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy3(:)-Policy3D(:))))
fprintf('%s lowmemory=3 (GI1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy3alt(:)-Policy3Dalt(:))));
...
fprintf('%s DC1_GI1 vs GI1, this should be zero: %.3e \n',qh,max(abs(V3(:)-V4(:))))
fprintf('%s DC1_GI1 vs GI1 (Valt), this should be zero: %.3e \n',qh,max(abs(V3alt(:)-V4alt(:))))
fprintf('%s DC1_GI1 vs GI1 (Policy), this should be zero: %.3e \n',qh,max(abs(Policy3(:)-Policy4(:))))
fprintf('%s DC1_GI1 vs GI1 (Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy3alt(:)-Policy4alt(:))));
% ValueFnFromPolicy oracle on DC1_GI1 (routes to the SemiExo-GI VFP via vfoptions.gridinterplayer)
...
fprintf('%s ValueFnFromPolicy (DC1_GI1), this should be zero: %.3e \n',qh,max(abs(V4fromPolicy(:)-V4(:))))
fprintf('%s ValueFnFromPolicy (DC1_GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V4altfromPolicy(:)-V4alt(:))))
...
fprintf('%s lowmemory=1 (DC1_GI1), this should be zero: %.3e \n',qh,max(abs(V4(:)-V4B(:))))
fprintf('%s lowmemory=1 (DC1_GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V4alt(:)-V4Balt(:))))
fprintf('%s lowmemory=1 (DC1_GI1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy4(:)-Policy4B(:))))
fprintf('%s lowmemory=1 (DC1_GI1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy4alt(:)-Policy4Balt(:))));
...
fprintf('%s lowmemory=2 (DC1_GI1), this should be zero: %.3e \n',qh,max(abs(V4(:)-V4C(:))))
fprintf('%s lowmemory=2 (DC1_GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V4alt(:)-V4Calt(:))))
fprintf('%s lowmemory=2 (DC1_GI1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy4(:)-Policy4C(:))))
fprintf('%s lowmemory=2 (DC1_GI1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy4alt(:)-Policy4Calt(:))));
...
fprintf('%s lowmemory=3 (DC1_GI1), this should be zero: %.3e \n',qh,max(abs(V4(:)-V4D(:))))
fprintf('%s lowmemory=3 (DC1_GI1, Valt), this should be zero: %.3e \n',qh,max(abs(V4alt(:)-V4Dalt(:))))
fprintf('%s lowmemory=3 (DC1_GI1, Policy), this should be zero: %.3e \n',qh,max(abs(Policy4(:)-Policy4D(:))))
fprintf('%s lowmemory=3 (DC1_GI1, Policyalt), this should be zero: %.3e \n',qh,max(abs(Policy4alt(:)-Policy4Dalt(:))));
