...
% Model A: 'fake' experienceassetze that ignores both z and e
vfoptionsA=struct();
...
% Model B: plain experienceasset (with z+e shocks still present)
vfoptionsB=struct();
...
fprintf('CrossTest3 (fake-both-ignored experienceassetze vs plain experienceasset), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
