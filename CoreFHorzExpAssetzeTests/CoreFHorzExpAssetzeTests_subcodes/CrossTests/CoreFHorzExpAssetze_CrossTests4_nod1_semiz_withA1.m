function output=CoreFHorzExpAssetze_CrossTests4_nod1_semiz_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 4 for experienceassetze+semiz: experienceassetze+semiz where z is iid-markov (transition
% matrix has all rows identical) compared to experienceassete+semiz with a 2-dim e (n_e=[n_z, n_e_orig])
% where e1 takes the z values and e2 takes the original e values. The two models are mathematically
% the same problem; only the asset-machinery dispatch differs.
%
...
% ReturnFn for Model A: uses (semiz,z,e); existing semiz file
ReturnFn_A=@(d2,d3,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetze_nod1_z_e_semiz(d2,d3,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
% ReturnFn for Model B: inline wrapper; (e1,e2) play the roles of (z,e)
ReturnFn_B=@(d2,d3,a1prime,a1,a2,semiz,e1,e2,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_ExpAssetze_nod1_z_e_semiz(d2,d3,a1prime,a1,a2,semiz,e1,e2,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
...
% Model A: experienceassetze+semiz with iid-markov z + normal iid e
...
% Model B: experienceassete+semiz with 2-dim e (e1=z values, e2=e values)
n_e_B=[n_z, n_e_orig];
...
fprintf('CrossTest4+semiz (experienceassetze iid-markov-z + e vs experienceassete with 2-dim e; nod1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
