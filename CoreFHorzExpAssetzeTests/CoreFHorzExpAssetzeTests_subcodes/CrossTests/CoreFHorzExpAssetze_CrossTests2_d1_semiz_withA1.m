function output=CoreFHorzExpAssetze_CrossTests2_d1_semiz_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 2 for experienceassetze+semiz (d1 version): 'fake' experienceassetze+semiz whose
% aprimeFn ignores z (becomes effectively experienceassete+semiz), vs actual experienceassete+semiz.
...
fprintf('CrossTest2+semiz (fake-z-ignored experienceassetze vs experienceassete; d1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
