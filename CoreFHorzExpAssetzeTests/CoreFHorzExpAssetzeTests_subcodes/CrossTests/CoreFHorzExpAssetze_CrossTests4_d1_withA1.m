...
ReturnFn_B=@(d1,d2,a1prime,a1,a2,e1,e2,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension) ReturnFn_ExpAssetze_d1_z_e(d1,d2,a1prime,a1,a2,e1,e2,r,w,kappa_j,sigma,varphi,eta,agej,Jr,pension);
...
% Model B: experienceassete with 2-dim e
n_e_B=[n_z, n_e_orig];
...
jequaloneDist_B=zeros([n_a,n_z,n_e_orig],'gpuArray');
...
fprintf('CrossTest4 (experienceassetze iid-markov-z + e vs experienceassete with 2-dim e; d1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
