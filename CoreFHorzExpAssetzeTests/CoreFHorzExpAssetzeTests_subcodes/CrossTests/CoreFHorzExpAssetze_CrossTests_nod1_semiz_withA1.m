...
% Model A: 'fake' experienceassetze+semiz that ignores e
...
% Model B: experienceassetz+semiz (with same e shock present, but aprimeFn doesn't depend on e)
...
fprintf('CrossTest1+semiz (fake-e-ignored experienceassetze vs experienceassetz; nod1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
