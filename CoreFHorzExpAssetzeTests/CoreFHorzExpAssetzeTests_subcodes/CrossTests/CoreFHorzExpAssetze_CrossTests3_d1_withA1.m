function output=CoreFHorzExpAssetze_CrossTests3_d1_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)

% Cross-test 3 for experienceassetze (d1 version): 'fake' experienceassetze that ignores both z and e
% (effectively plain experienceasset) vs actual experienceasset. Both have z+e shocks.
...
% Model A: 'fake' experienceassetze that ignores both
vfoptionsA=struct();
...
fprintf('CrossTest3 (fake-both-ignored experienceassetze vs plain experienceasset; d1), this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n', max(abs(V_A(:)-V_B(:))), max(abs(Policy_A(:)-Policy_B(:))), max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
