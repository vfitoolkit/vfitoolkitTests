function output=CoreFHorzExpAssetze_CrossTests6_nod1_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% Cross-test 4 (nod1, with2A1): a DEGENERATE second standard asset a1_2 (single grid point {0})
% must reduce the two-standard-asset model back to the plain single-standard-asset withA1 model.
% Follows CoreFHorzExpAsset_CrossTests_nod1_nosemiz_with2A1, adapted to experienceassetze
% (z and e both always present; aprimeFn(d2,a2,z,e,...)); uses the richest shock case: z and e.
...
vfoptionsA=struct(); vfoptionsA.experienceassetze=1; vfoptionsA.aprimeFn=aprimeFn;
vfoptionsA.n_e=vfoptionsbaseline.n_e; vfoptionsA.e_grid=vfoptionsbaseline.e_grid; vfoptionsA.pi_e=vfoptionsbaseline.pi_e;
simoptionsA=struct(); simoptionsA.experienceassetze=1; simoptionsA.aprimeFn=aprimeFn; simoptionsA.d_grid=d_grid; simoptionsA.a_grid=a_grid;
...
ReturnFn_B=@(d2,a1prime,a1_2prime,a1,a1_2,a2,z,e,r,r2,w,kappa_j,sigma,agej,Jr,pension) ReturnFn_ExpAssetze_nod1_z_e_with2A1(d2,a1prime,a1_2prime,a1,a1_2,a2,z,e,r,r2,w,kappa_j,sigma,agej,Jr,pension);
vfoptionsB=struct(); vfoptionsB.experienceassetze=1; vfoptionsB.aprimeFn=aprimeFn;
vfoptionsB.n_e=vfoptionsbaseline.n_e; vfoptionsB.e_grid=vfoptionsbaseline.e_grid; vfoptionsB.pi_e=vfoptionsbaseline.pi_e;
simoptionsB=struct(); simoptionsB.experienceassetze=1; simoptionsB.aprimeFn=aprimeFn; simoptionsB.d_grid=d_grid; simoptionsB.a_grid=a_grid_B;
...
fprintf('Cross test 6 (with2A1, nod1, z e): degenerate a1_2 reduces to withA1, this should be zero: V %2.8f, Dist %2.8f \n',max(abs(V_A(:)-V_B(:))),max(abs(StationaryDist_A(:)-StationaryDist_B(:))))
