function output=CoreFHorzExpAssetze_d1_z_e_semiz_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% ExpAssetze noa1 + semiz: with d1 (labour), z, e. Experience asset a2 is the only endogenous state.
...
% PENDING TOOLKIT SUPPORT: ExpAssetzeSemiExo noa1 raws do not yet exist, so this
% errors at the ValueFnIter call (test-first: written ahead of the toolkit code).
...
FnsToEvaluate.assets=@(d1,d2,d3,a2,semiz,z,e) a2;
FnsToEvaluate.earnings=@(d1,d2,d3,a2,semiz,z,e,w,kappa_j) w*kappa_j*d1*d2*a2*semiz*z*e;
