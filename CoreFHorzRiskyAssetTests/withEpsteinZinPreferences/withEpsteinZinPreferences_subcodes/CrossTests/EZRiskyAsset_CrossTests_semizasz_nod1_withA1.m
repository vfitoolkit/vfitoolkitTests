...
for ezcase=1:3
    ...
    fprintf('CrossTest semizasz (nod1 noe, withA1) [EZ %s]: should be zero: V %2.8f, Policy(riskyshare,savings) %2.8f, Dist %2.8f \n',casestr,max(abs(V_z(:)-V_s(:))),max(abs(Pd_z(:)-Pd_s(:))),max(abs(StationaryDist_z(:)-StationaryDist_s(:))))
    ...
    Pd_z=Policy_z(1:2,:); Pd_s=Policy_s(1:2,:);
    fprintf('CrossTest semizasz (nod1 e, withA1) [EZ %s]: should be zero: V %2.8f, Policy(riskyshare,savings) %2.8f, Dist %2.8f \n',casestr,max(abs(V_z(:)-V_s(:))),max(abs(Pd_z(:)-Pd_s(:))),max(abs(StationaryDist_z(:)-StationaryDist_s(:))))
