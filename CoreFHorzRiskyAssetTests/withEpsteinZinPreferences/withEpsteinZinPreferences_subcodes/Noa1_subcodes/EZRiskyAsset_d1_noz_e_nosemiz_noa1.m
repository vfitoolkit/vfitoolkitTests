function output=EZRiskyAsset_d1_noz_e_nosemiz_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Epstein-Zin mirror of CoreFHorzRiskyAsset_d1_noz_e_nosemiz_noa1.m. The model is run three times:
...
% ONE joint CE over (u, eprime) — the gamma=1/phi collapse only holds exactly if the
...
ReturnFn_cons=@(h,savings,a,e,r,w,kappa_j,varphi,agej,Jr,pension) EZRiskyReturnFn_cons_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,varphi,agej,Jr,pension);
ReturnFn_posU=@(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension) EZRiskyReturnFn_positiveUtils_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
ReturnFn_negU=@(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension) EZRiskyReturnFn_negativeUtils_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);

% With riskyasset, need to include all d in FnsToEvaluate, even if they are not in the ReturnFn
FnsToEvaluate.assets=@(h,riskyshare,savings,a,e) a;
FnsToEvaluate.savings=@(h,riskyshare,savings,a,e) savings;
FnsToEvaluate.hours=@(h,riskyshare,savings,a,e) h;
FnsToEvaluate.earnings=@(h,riskyshare,savings,a,e,w,kappa_j) w*kappa_j*h*e;
...
title('EZ cons-units, age-cond mean: assets / savings (d1+e)')
...
subplot(2,1,1); plot(1:1:N_j, AgeConditionalStats1.assets.Mean, 1:1:N_j, AgeConditionalStats1.savings.Mean)
title('EZ positive utils, age-cond mean: assets / savings (d1+e)')
legend('assets','savings')
subplot(2,1,2); plot(1:1:N_j, AgeConditionalStats1.hours.Mean, 1:1:N_j, AgeConditionalStats1.earnings.Mean)
...
subplot(2,1,1); plot(1:1:N_j, AgeConditionalStats1.assets.Mean, 1:1:N_j, AgeConditionalStats1.savings.Mean)
title('EZ negative utils, age-cond mean: assets / savings (d1+e)')
legend('assets','savings')
subplot(2,1,2); plot(1:1:N_j, AgeConditionalStats1.hours.Mean, 1:1:N_j, AgeConditionalStats1.earnings.Mean)
...
% consumption-units
ReturnFn_cons_scaled=@(h,savings,a,e,r,w,kappa_j,varphi,agej,Jr,pension,ezscalefactor) ezscalefactor*EZRiskyReturnFn_cons_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=0;
vfoptions1.EZriskaversion='ezgamma';
...
ReturnFn_posU_scaled=@(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru) ezscalefactoru*EZRiskyReturnFn_positiveUtils_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=1;
...
ReturnFn_negU_scaled=@(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru) ezscalefactoru*EZRiskyReturnFn_negativeUtils_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=0;
...
% sections above (lowmemory=0 and 1). Run for all three EZ cases.
...
for ezcase=1:3
    ...
    for jstar=[round(3*N_j/4),N_j]
        ...
        [Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
        fprintf('V_Jplus1 (jstar=%i), lowmemory=1 [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Vbase(:)-Vshort(:))))
        fprintf('V_Jplus1 (jstar=%i), lowmemory=1 [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Policybase(:)-Policyshort(:))))
...
fprintf('EZ gamma=1/phi collapse with sj (lowmemory=1), Policy [EZ cons-units]: should give zero: %.3e \n',max(abs(Policy1c(:)-Policy1d(:))))
...
% rungs (lowmemory=0 and 1); noa1 riskyasset has no DC/GI methods to cross.
for ezcase=1:3
    vfoptionsv=vfoptions; % inherits all the riskyasset settings (refine_d, aprimeFn, n_u, u_grid, pi_u) and the e-shock (n_e, e_grid, pi_e)
    vfoptionsv.exoticpreferences='EpsteinZin';
    if ezcase==1 % consumption-units (traditional Epstein-Zin)
    ...
    fprintf('sj, lowmemory=1 [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V2(:))))
...
% cases: FromPolicy, plus the file's lowmemory rungs (lowmemory=0 and 1).
for ezcase=1:3
    vfoptionsv=vfoptions; % inherits all the riskyasset settings (refine_d, aprimeFn, n_u, u_grid, pi_u) and the e-shock (n_e, e_grid, pi_e)
    vfoptionsv.exoticpreferences='EpsteinZin';
    if ezcase==1 % consumption-units (traditional Epstein-Zin)
...
for ezcase=1:3
    ...
    vfoptionsjs.lowmemory=1;
    [Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
    fprintf('V_Jplus1 with sj+warm-glow (jstar=%i), lowmemory=1 [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Vbase(:)-Vshort(:))))
    fprintf('V_Jplus1 with sj+warm-glow (jstar=%i), lowmemory=1 [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Policybase(:)-Policyshort(:))))
    vfoptionsjs.lowmemory=0;
end
...
% ^ezc8 sites. All three EZ cases: FromPolicy, plus the file's lowmemory rungs (lowmemory=0 and 1).
for ezcase=1:3
    vfoptionsv=vfoptions; % inherits all the riskyasset settings (refine_d, aprimeFn, n_u, u_grid, pi_u) and the e-shock (n_e, e_grid, pi_e)
    vfoptionsv.exoticpreferences='EpsteinZin';
    if ezcase==1 % consumption-units (traditional Epstein-Zin)
    ...
    vfoptionsv.lowmemory=1;
    [V2,Policy2]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,[],vfoptionsv);
    fprintf('EZmortalityriskaversion distinct, lowmemory=1 [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V2(:))))
    fprintf('EZmortalityriskaversion distinct, lowmemory=1 [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy1(:)-Policy2(:))))
    vfoptionsv.lowmemory=0;
end
clear V1 V2 Policy1 Policy2 V1fromPolicy
...
% consumption-units
ReturnFn_cons_scaled2=@(h,savings,a,e,r,w,kappa_j,varphi,agej,Jr,pension,ezscalefactor2) ezscalefactor2*EZRiskyReturnFn_cons_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=0;
vfoptions1.EZriskaversion='ezgamma';
...
% utility-units, positive
ReturnFn_posU_scaled2=@(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru2) ezscalefactoru2*EZRiskyReturnFn_positiveUtils_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=1;
...
% utility-units, negative
ReturnFn_negU_scaled2=@(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru2) ezscalefactoru2*EZRiskyReturnFn_negativeUtils_d1_noz_e_nosemiz(h,savings,a,e,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=0;
