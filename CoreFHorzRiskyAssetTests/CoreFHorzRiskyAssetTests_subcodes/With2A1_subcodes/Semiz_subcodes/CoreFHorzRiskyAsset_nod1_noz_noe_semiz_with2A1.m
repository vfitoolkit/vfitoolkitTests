function output=CoreFHorzRiskyAsset_nod1_noz_noe_semiz_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% RiskyAsset, with TWO standard endogenous assets a1_1,a1_2 plus a2 (risky).
% Exercises the DC2A / GI2A / DC2A_GI2A code paths (length(n_a1)>1). Variant: nod1_noz_noe_semiz.
...
ReturnFn=@(savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz,r,w,kappa_j,sigma,r_a1,r_a1_2,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_noe_semiz_with2A1(savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz,r,w,kappa_j,sigma,r_a1,r_a1_2,agej,Jr,pension,uempbenefit,searcheffortcost);

% With riskyasset, need to include all d in FnsToEvaluate, even if they are not in the ReturnFn
FnsToEvaluate.a1=@(riskyshare,savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz) a1;
FnsToEvaluate.a1_2=@(riskyshare,savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz) a1_2;
FnsToEvaluate.a2=@(riskyshare,savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz) a2;
FnsToEvaluate.savings=@(riskyshare,savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz) savings;
FnsToEvaluate.earnings=@(riskyshare,savings,dsemiz,a1prime,a1_2prime,a1,a1_2,a2,semiz,w,kappa_j) w*kappa_j;
