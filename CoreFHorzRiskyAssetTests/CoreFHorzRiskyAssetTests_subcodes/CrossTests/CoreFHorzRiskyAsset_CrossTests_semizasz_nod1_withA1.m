function output=CoreFHorzRiskyAsset_CrossTests_semizasz_nod1_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% CrossTest (semiz-as-z): RiskyAsset, nod1, WITH a1.
...
ReturnFn_z=@(savings,a1prime,a1,a2,z,r,w,kappa_j,sigma,r_a1,agej,Jr,pension) ReturnFn_nod1_z_noe_nosemiz_withA1(savings,a1prime,a1,a2,z,r,w,kappa_j,sigma,r_a1,agej,Jr,pension);
ReturnFn_s=@(savings,dsemiz,a1prime,a1,a2,semiz,r,w,kappa_j,sigma,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_noe_semiz_withA1(savings,dsemiz,a1prime,a1,a2,semiz,r,w,kappa_j,sigma,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
...
fprintf('CrossTest semizasz (nod1 noe, withA1): should be zero: V %2.8f, Policy(riskyshare,savings) %2.8f, Dist %2.8f \n',max(abs(V_z(:)-V_s(:))),max(abs(Pd_z(:)-Pd_s(:))),max(abs(StationaryDist_z(:)-StationaryDist_s(:))))
...
ReturnFn_z=@(savings,a1prime,a1,a2,z,e,r,w,kappa_j,sigma,r_a1,agej,Jr,pension) ReturnFn_nod1_z_e_nosemiz_withA1(savings,a1prime,a1,a2,z,e,r,w,kappa_j,sigma,r_a1,agej,Jr,pension);
ReturnFn_s=@(savings,dsemiz,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_e_semiz_withA1(savings,dsemiz,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
...
Pd_z=Policy_z(1:2,:); Pd_s=Policy_s(1:2,:);
fprintf('CrossTest semizasz (nod1 e, withA1): should be zero: V %2.8f, Policy(riskyshare,savings) %2.8f, Dist %2.8f \n',max(abs(V_z(:)-V_s(:))),max(abs(Pd_z(:)-Pd_s(:))),max(abs(StationaryDist_z(:)-StationaryDist_s(:))))
