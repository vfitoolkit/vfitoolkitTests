function output=CoreFHorzRiskyAsset_CrossTests_zase_d1_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% CrossTest (z-as-e): RiskyAsset, d1, nosemiz, WITH a1.
% As CoreFHorzRiskyAsset_CrossTests_zase_nod1 but with a d1 (labour supply h) AND a safe asset a1.
%   n_d=[h,riskyshare,savings]. a=[a1,a2]: a1=safe asset, a2=risky asset. n_a=[n_a1,n_a2].
...
ReturnFn_none=@(h,savings,a1prime,a1,a2,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension) ReturnFn_d1_noz_noe_nosemiz_withA1(h,savings,a1prime,a1,a2,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension);
...
ReturnFn_e=@(h,savings,a1prime,a1,a2,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension) ReturnFn_d1_noz_e_nosemiz_withA1(h,savings,a1prime,a1,a2,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension);
ReturnFn_ze=@(h,savings,a1prime,a1,a2,z,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension) ReturnFn_d1_z_e_nosemiz_withA1(h,savings,a1prime,a1,a2,z,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension);
...
fprintf('CrossTest zase (A, d1 withA1) single-point z == no z, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V0(:)-V0z(:))),max(abs(Policy0(:)-Policy0z(:))),max(abs(StationaryDist0(:)-StationaryDist0z(:))))
...
fprintf('CrossTest zase (B, d1 withA1) iid-markov-z == e, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V2(:))),max(abs(Policy1(:)-Policy2(:))),max(abs(StationaryDist1(:)-StationaryDist2(:))))
...
fprintf('CrossTest zase (C, d1 withA1) z&e with z=1, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V3(:))),max(abs(Policy1(:)-Policy3(:))),max(abs(StationaryDist1(:)-StationaryDist3(:))))
...
fprintf('CrossTest zase (C, d1 withA1) z&e with e=1, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V4(:))),max(abs(Policy1(:)-Policy4(:))),max(abs(StationaryDist1(:)-StationaryDist4(:))))
