function output=CoreFHorzRiskyAsset_CrossTests_ExpAssetu_nod1_semiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% CrossTest: RiskyAsset vs a degenerate ExperienceAssetu (nod1, +semiz, single asset).
% As CoreFHorzRiskyAsset_CrossTests_ExpAssetu_nod1 but BOTH sides carry a genuine employment
% semiz (dsemiz is a real decision on both sides, kept identical, nothing zeroed).
%   decisions : n_d=[n_d2,n_d3,n_d4]=[riskyshare,savings,dsemiz] on both sides, same d_grid, same order
%   R: refine_d=[0,1,1,1]        (riskyshare aprimeFn-only, savings both, dsemiz the semiz decision)
%   E: l_dexperienceassetu=2     (riskyshare+savings feed aprimeFn; trailing dsemiz is the semiz decision)
%   R aprimeFn: @(riskyshare,savings,u,r)
%   E aprimeFn: @(riskyshare,savings,a2,u,r)   -- ignores a2
%   R ReturnFn: @(savings,dsemiz,a,semiz,...)
%   E ReturnFn: @(riskyshare,savings,dsemiz,a,semiz,...)   -- ignores riskyshare
...
vfoptionsR.n_semiz=vfoptionsbaseline.n_semiz;
vfoptionsR.semiz_grid=vfoptionsbaseline.semiz_grid;
vfoptionsR.SemiExoStateFn=vfoptionsbaseline.SemiExoStateFn;
simoptionsR=struct();
...
simoptionsR.n_semiz=simoptionsbaseline.n_semiz;
simoptionsR.semiz_grid=simoptionsbaseline.semiz_grid;
simoptionsR.SemiExoStateFn=simoptionsbaseline.SemiExoStateFn;
...
vfoptionsE.l_dexperienceassetu=2; % riskyshare+savings feed the aprimeFn; trailing dsemiz is the semiz decision
...
vfoptionsE.n_semiz=vfoptionsbaseline.n_semiz;
vfoptionsE.semiz_grid=vfoptionsbaseline.semiz_grid;
...
simoptionsE.n_semiz=simoptionsbaseline.n_semiz;
simoptionsE.semiz_grid=simoptionsbaseline.semiz_grid;
simoptionsE.SemiExoStateFn=simoptionsbaseline.SemiExoStateFn;
...
ReturnFn_R=@(savings,dsemiz,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_noe_semiz(savings,dsemiz,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_E=@(riskyshare,savings,dsemiz,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_noe_semiz(savings,dsemiz,a,semiz,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
FnsR.assets=@(riskyshare,savings,dsemiz,a,semiz) a;
FnsE.assets=@(riskyshare,savings,dsemiz,a,semiz) a;
...
ReturnFn_R=@(savings,dsemiz,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_e_semiz(savings,dsemiz,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_E=@(riskyshare,savings,dsemiz,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_nod1_noz_e_semiz(savings,dsemiz,a,semiz,e,r,w,kappa_j,sigma,agej,Jr,pension,uempbenefit,searcheffortcost);
FnsR.assets=@(riskyshare,savings,dsemiz,a,semiz,e) a;
FnsE.assets=@(riskyshare,savings,dsemiz,a,semiz,e) a;
...
fprintf('CrossTest ExpAssetu (nod1 noz e, +semiz): should be zero: V %2.8f, Policy %2.8f, Dist %2.8f, AllStats.Mean %2.8f \n',max(abs(VR(:)-VE(:))),max(abs(PolicyR(:)-PolicyE(:))),max(abs(StationaryDistR(:)-StationaryDistE(:))),abs(AllStatsR.assets.Mean-AllStatsE.assets.Mean))
