function output=CoreFHorzRiskyAsset_CrossTests_zase_d1_semiz_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% CrossTest (z-as-e) WITH a semiz background: RiskyAsset, d1, WITH a1.
% Same three z-as-e equivalences as zase_nod1_semiz, but with a d1 (labour supply h)
% and a standard safe asset a1.
% a=[a1,a2]: a1=safe asset (a1prime chosen directly), a2=risky asset (set by aprimeFn).
% n_a=[n_a1,n_a2]; a_grid=[a1_grid;a2_grid]. Every model also carries the (employment)
...
ReturnFn_none=@(h,savings,dsemiz,a1prime,a1,a2,semiz,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_noz_noe_semiz_withA1(h,savings,dsemiz,a1prime,a1,a2,semiz,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_z=@(h,savings,dsemiz,a1prime,a1,a2,semiz,z,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_z_noe_semiz_withA1(h,savings,dsemiz,a1prime,a1,a2,semiz,z,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_e=@(h,savings,dsemiz,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_noz_e_semiz_withA1(h,savings,dsemiz,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
ReturnFn_ze=@(h,savings,dsemiz,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_z_e_semiz_withA1(h,savings,dsemiz,a1prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
...
fprintf('CrossTest zase+semiz (A, d1+withA1) single-point z == no z, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V0(:)-V0z(:))),max(abs(Policy0(:)-Policy0z(:))),max(abs(StationaryDist0(:)-StationaryDist0z(:))))
...
fprintf('CrossTest zase+semiz (B, d1+withA1) iid-markov-z == e, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V2(:))),max(abs(Policy1(:)-Policy2(:))),max(abs(StationaryDist1(:)-StationaryDist2(:))))
...
fprintf('CrossTest zase+semiz (C, d1+withA1) z&e with z=1, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V3(:))),max(abs(Policy1(:)-Policy3(:))),max(abs(StationaryDist1(:)-StationaryDist3(:))))
...
fprintf('CrossTest zase+semiz (C, d1+withA1) z&e with e=1, this should be zero: V %2.8f, Policy %2.8f, Dist %2.8f \n',max(abs(V1(:)-V4(:))),max(abs(Policy1(:)-Policy4(:))),max(abs(StationaryDist1(:)-StationaryDist4(:))))
