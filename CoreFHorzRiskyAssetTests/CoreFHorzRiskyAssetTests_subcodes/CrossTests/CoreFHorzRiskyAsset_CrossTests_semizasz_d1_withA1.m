function output=CoreFHorzRiskyAsset_CrossTests_semizasz_d1_withA1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline)
% CrossTest (semiz-as-z): RiskyAsset, d1, WITH a1.
...
jequaloneDist_s=zeros([n_a,vfoptions_s.n_semiz],'gpuArray'); jequaloneDist_s(1,1,1)=1;

ReturnFn_z=@(h,savings,a1prime,a1,a2,z,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension) ReturnFn_d1_z_noe_nosemiz_withA1(h,savings,a1prime,a1,a2,z,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension);
ReturnFn_s=@(h,savings,dsemiz,a1prime,a1,a2,semiz,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_noz_noe_semiz_withA1(h,savings,dsemiz,a1prime,a1,a2,semiz,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
...
fprintf('CrossTest semizasz (d1 noe, withA1): should be zero: V %2.8f, Policy(h,riskyshare,savings) %2.8f, Dist %2.8f \n',max(abs(V_z(:)-V_s(:))),max(abs(Pd_z(:)-Pd_s(:))),max(abs(StationaryDist_z(:)-StationaryDist_s(:))))
...
jequaloneDist_s=zeros([n_a,vfoptions_s.n_semiz,vfoptions_s_e.n_e],'gpuArray'); jequaloneDist_s(1,1,1,ceil(vfoptions_s_e.n_e/2))=1;

ReturnFn_z=@(h,savings,a1prime,a1,a2,z,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension) ReturnFn_d1_z_e_nosemiz_withA1(h,savings,a1prime,a1,a2,z,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension);
ReturnFn_s=@(h,savings,dsemiz,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_noz_e_semiz_withA1(h,savings,dsemiz,a1prime,a1,a2,semiz,e,r,w,kappa_j,sigma,eta,varphi,r_a1,agej,Jr,pension,uempbenefit,searcheffortcost);
...
Pd_z=Policy_z(1:3,:); Pd_s=Policy_s(1:3,:);
fprintf('CrossTest semizasz (d1 e, withA1): should be zero: V %2.8f, Policy(h,riskyshare,savings) %2.8f, Dist %2.8f \n',max(abs(V_z(:)-V_s(:))),max(abs(Pd_z(:)-Pd_s(:))),max(abs(StationaryDist_z(:)-StationaryDist_s(:))))
