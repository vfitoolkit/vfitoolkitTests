function output=CoreFHorzRiskyAsset_d1_z_e_semiz_noa1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% RiskyAsset + semiz, with d1 (labour), z, e.
...
ReturnFn=@(h,savings,dsemiz,a,semiz,z,e,r,w,kappa_j,sigma,eta,varphi,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_z_e_semiz(h,savings,dsemiz,a,semiz,z,e,r,w,kappa_j,sigma,eta,varphi,agej,Jr,pension,uempbenefit,searcheffortcost);
...
%%
clear V1 V1B V1C V1D Policy1 Policy1B Policy1C Policy1D PolicyVals1 V1fromPolicy
...
[Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1 (jstar=%i), lowmemory=3, this should be zero: %2.8f \n',jstar,max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1 (jstar=%i), lowmemory=3, this should be zero: %2.8f \n',jstar,max(abs(Policybase(:)-Policyshort(:))))
vfoptionsjs.lowmemory=0;
...
title('Age-conditional mean: assets / savings (d1+semiz+z+e)')
