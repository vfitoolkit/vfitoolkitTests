function output=CoreInfHorzTPath_nod_z_e_nosemiz(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline,n_d_GE,n_a_GE,d_grid_GE,a_grid_GE,figure_c)
...
FnsToEvaluate.earnings=@(aprime,a,z,e,w) w*z*e;
FnsToEvaluate.nextassets=@(aprime,a,z,e) aprime; % aprime-dependent, so it tests the policy decode directly
FnsToEvaluate.zval=@(aprime,a,z,e) z; % the markov shock itself
FnsToEvaluate.eval=@(aprime,a,z,e) e; % the iid shock itself (kept fixed here, so it acts as a control)
...
FnsToEvaluateGE.K=@(aprime,a,z,e) a;
FnsToEvaluateGE.N=@(aprime,a,z,e) z*e;
