function output=CoreInfHorzTPath_nod_z_noe_nosemiz_with2A(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline,n_d_GE,n_a_GE,d_grid_GE,a_grid_GE,figure_c)
% Two-endogenous-state InfHorz TPath test, mirroring the fig-3 (nod, z) battery.
% Model: simplified Kitao (2008), no taxes -- the same entrepreneur model that CoreInfHorzTests
% uses for its 2A subcodes. Endogenous states: asset, occupation (0=worker, 1=entrepreneur).
% Exogenous markov z: eta (labor productivity), theta (entrepreneurial ability).
% n_a(2)=2 is what triggers the DC2A/GI2A code paths.
%
...
% Setup some FnsToEvaluate (functions of (aprime,eprime,a,e,eta,theta))
...
fprintf('with2A: Divide-and-conquer, this should be zero: %.3e \n',max(abs(VPath1(:)-VPath2(:))))
...
fprintf('with2A: low memory, this should be zero: %.3e \n',max(abs(VPath1(:)-VPath1B(:))))
...
fprintf('with2A: TPath vs FHorz (with DC, no GI), this should be zero, V: %.3e \n',max(abs(dev(:))))
temp1=reshape(PolicyPath2,[],T); temp2=reshape(Policy_FH,[],N_jFH);
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC, no GI), this should be zero, Policy: %.3e \n',max(abs(dev(:))))
...
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC, no GI), this should be zero, PolicyVals: %.3e \n',max(abs(dev(:))))
...
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC, no GI), this should be zero, AgentDist: %.3e \n',max(abs(dev(:))))
...
for fnname={'assets','entrepreneur','nextassets','etaval','thetaval'}
    ...
    fprintf('with2A: TPath vs FHorz (with DC, no GI), this should be zero, AggVars %s Mean: %.3e \n',fnname{1},max(abs(dev(:))))
end
for statname={'Mean','Median','StdDeviation','Variance','Gini','LorenzCurve','QuantileCutoffs','QuantileMeans'}
    ...
    dev=temp1(:,1:N_jFH)-temp2;
    fprintf('with2A: TPath vs FHorz (with DC, no GI), this should be zero, AllStats assets %s: %.3e \n',statname{1},max(abs(dev(:))))
end
...
fprintf('with2A: the kron decomposition of pi_z, this should be zero: %.3e \n',max(abs(pi_z(:)-reshape(kron(pi_theta,pi_eta_base),[],1))))
...
z_grid_KFTT=[eta_grid_KFTT(:,1:T); repmat(theta_grid,1,T)]; % [sum(n_z),T] stacked; only eta moves
pi_z_KFTT=zeros(prod(n_z),prod(n_z),T); % KFTT gives Jz-1=T transition matrices
for tt=1:T
...
fprintf('with2A: Divide-and-conquer (with GI), this should be zero: %.3e \n',max(abs(VPath3(:)-VPath4(:))))
...
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC and GI), this should be zero, V: %.3e \n',max(abs(dev(:))))
temp1=reshape(PolicyPath4,[],T); temp2=reshape(Policy_FH,[],N_jFH);
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC and GI), this should be zero, Policy: %.3e \n',max(abs(dev(:))))
...
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC and GI), this should be zero, PolicyVals: %.3e \n',max(abs(dev(:))))
...
dev=temp1(:,1:N_jFH)-temp2;
fprintf('with2A: TPath vs FHorz (with DC and GI), this should be zero, AgentDist: %.3e \n',max(abs(dev(:))))
...
for fnname={'assets','entrepreneur','nextassets','etaval','thetaval'}
    dev=AggVarsPathFH.(fnname{1}).Mean(1:N_jFH)-AgeCondStats_FH.(fnname{1}).Mean;
    fprintf('with2A: TPath vs FHorz (with DC and GI), this should be zero, AggVars %s Mean: %.3e \n',fnname{1},max(abs(dev(:))))
end
for statname={'Mean','Median','StdDeviation','Variance','Gini','LorenzCurve','QuantileCutoffs','QuantileMeans'}
    temp1=AllStatsPathFH.assets.(statname{1}); temp2=AgeCondStats_FH.assets.(statname{1});
    dev=temp1(:,1:N_jFH)-temp2;
    fprintf('with2A: TPath vs FHorz (with DC and GI), this should be zero, AllStats assets %s: %.3e \n',statname{1},max(abs(dev(:))))
end
...
fprintf('with2A: AgentDist along TPath with/without grid interp, this should be close to zero: %.3e \n',max(abs(AgentDistPath2(:)-AgentDistPath4(:))))
...
fprintf('with2A: Constant TPath (no DC, no GI), this should be zero, V: %.3e \n',max(abs(VPath1(:)-Vfin_rep(:))))
Polfin_rep=repmat(Policy_final,1,1,1,1,1,T);
fprintf('with2A: Constant TPath (no DC, no GI), this should be zero, Policy: %.3e \n',max(abs(PolicyPath1(:)-Polfin_rep(:))))
...
fprintf('with2A: Constant TPath (no DC, no GI), this should be zero, PolicyVals: %.3e \n',max(abs(PolicyValsPath1(:)-PolicyValsfin_rep(:))))
AD_rep=repmat(AgentDist_initial,1,1,1,1,T);
fprintf('with2A: Constant TPath (no DC, no GI), this should be zero, AgentDist (note: tolerance=1e-6): %.3e \n',max(abs(AgentDistPath1(:)-AD_rep(:))))
...
fprintf('with2A: Constant TPath (with DC, no GI), this should be zero, V: %.3e \n',max(abs(VPath1(:)-Vfin_rep(:))))
fprintf('with2A: Constant TPath (with DC, no GI), this should be zero, Policy: %.3e \n',max(abs(PolicyPath1(:)-Polfin_rep(:))))
fprintf('with2A: Constant TPath (with DC, no GI), this should be zero, PolicyVals: %.3e \n',max(abs(PolicyValsPath1(:)-PolicyValsfin_rep(:))))
fprintf('with2A: Constant TPath (with DC, no GI), this should be zero, AgentDist (note: tolerance=1e-6): %.3e \n',max(abs(AgentDistPath1(:)-AD_rep(:))))
...
fprintf('with2A: Constant TPath (with DC and GI), this should be zero, V: %.3e \n',max(abs(VPath1(:)-Vfin_rep_GI(:))))
Polfin_rep_GI=repmat(Policy_final_GI,1,1,1,1,1,T);
fprintf('with2A: Constant TPath (with DC and GI), this should be zero, Policy: %.3e \n',max(abs(PolicyPath1(:)-Polfin_rep_GI(:))))
...
fprintf('with2A: Constant TPath (with DC and GI), this should be zero, PolicyVals: %.3e \n',max(abs(PolicyValsPath1(:)-PolicyValsfin_GI_rep(:))))
AD_rep_GI=repmat(AgentDist_initial_GI,1,1,1,1,T);
fprintf('with2A: Constant TPath (with DC and GI), this should be zero, AgentDist (note: tolerance=1e-6): %.3e \n',max(abs(AgentDistPath1(:)-AD_rep_GI(:))))
...
FnsToEvaluateGE.A=@(aprime,eprime,a,e,eta,theta) a; % total household assets
FnsToEvaluateGE.K_noncorp=@(aprime,eprime,a,e,eta,theta,r,w,delta,upsilon1,upsilon2,leverage,phi) EntreCapital_nod_with2A(aprime,eprime,a,e,theta,r,w,delta,upsilon1,upsilon2,leverage,phi); % capital inside entrepreneurs' own businesses
FnsToEvaluateGE.L_workers=@(aprime,eprime,a,e,eta,theta) (e==0)*eta; % labour supplied to the market (entrepreneurs' own eta goes to their own firm)
FnsToEvaluateGE.N_hired=@(aprime,eprime,a,e,eta,theta,r,w,delta,upsilon1,upsilon2,leverage,phi) EntreHiredLabour_nod_with2A(aprime,eprime,a,e,eta,theta,r,w,delta,upsilon1,upsilon2,leverage,phi); % labour entrepreneurs hire in
...
fprintf('with2A: One iter of TPath, big-grid with/without GI, this should be close to zero: %.3e \n',max(abs(PricePath3A.r-PricePath3B.r)))
