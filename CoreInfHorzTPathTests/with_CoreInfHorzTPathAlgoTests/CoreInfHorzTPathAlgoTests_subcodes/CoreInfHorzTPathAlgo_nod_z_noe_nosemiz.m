function output=CoreInfHorzTPathAlgo_nod_z_noe_nosemiz(T,PricePath,ParamPath,n_d,n_a,n_a_big,n_z,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,transpathoptionsbaseline,vfoptionsbaseline,simoptionsbaseline,n_d_GE,n_a_GE,d_grid_GE,a_grid_GE,figure_c)
...
% The model and its stationary general eqm are lifted from CoreInfHorzTPath_nod_z_noe_nosemiz.
...
FnsToEvaluate.nextassets=@(aprime,a,z) aprime; % aprime-dependent, so it tests the policy decode directly
FnsToEvaluate.zval=@(aprime,a,z) z; % the shock itself, so the path of its mean/std deviation tracks the z process
...
FnsToEvaluateGE.N=@(aprime,a,z) z;
...
% FullJacobianReuseVpath=1 restarts the backward pass at the perturbed period rather than redoing all
% of it. That is exact, not an approximation: the value fn at a period cannot depend on a price at any
% earlier period, so the later periods of the baseline solve are reused unchanged. The two must
% therefore agree to machine precision, not merely closely.
% maxiter=1 is all this needs: one Jacobian gets built and one step taken, so any difference in J
% shows up in the price path, and two complete FullJacobian solves are not paid for. Only done on this
% model, without d: on the model with d a FullJacobian solve is minutes rather than seconds.
...
tic;
[PricePathG1,~]=TransitionPath_InfHorz(PricePathBumped, ParamPathGE, T, V_finalGE, AgentDist_initialGE, n_d_GE, n_a_GE, n_z, d_grid_GE,a_grid_GE,z_grid, pi_z, ReturnFn, FnsToEvaluateGE, GeneralEqmEqnsGE, ParamsGE, DiscountFactorParamNames, transpathoptionsG, simoptions1, vfoptions1, []);
time_G1=toc;
transpathoptionsG.GEnewprice1.FullJacobianReuseVpath=1; % restart the backward pass at the perturbed period
tic;
[PricePathG2,~]=TransitionPath_InfHorz(PricePathBumped, ParamPathGE, T, V_finalGE, AgentDist_initialGE, n_d_GE, n_a_GE, n_z, d_grid_GE,a_grid_GE,z_grid, pi_z, ReturnFn, FnsToEvaluateGE, GeneralEqmEqnsGE, ParamsGE, DiscountFactorParamNames, transpathoptionsG, simoptions1, vfoptions1, []);
time_G2=toc;
fprintf('FullJacobian plain loop vs triangular, one iteration, this should be zero, r: %.3e, w: %.3e \n',max(abs(PricePathG2.r-PricePathG1.r)),max(abs(PricePathG2.w-PricePathG1.w)))
fprintf('Runtime, FullJacobian one iteration: plain loop %2.2f seconds, triangular %2.2f seconds \n',time_G1,time_G2)
