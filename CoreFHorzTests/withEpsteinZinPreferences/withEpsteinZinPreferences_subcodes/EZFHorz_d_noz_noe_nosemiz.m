function output=EZFHorz_d_noz_noe_nosemiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Epstein-Zin mirror of QHDFHorz_d_noz_noe_nosemiz.m (which mirrors CoreFHorz_d_noz_noe_nosemiz.m).
...
% identity, so the gamma=1/phi collapse here additionally isolates the TIME-aggregator
% (it holds for ANY EZriskaversion when there is no risk). No lowmemory levels (per
% docs/ExogenousShocks.md, no shocks means lowmemory=0 only).
...
jequaloneDist(1)=1; % no assets, midpoint shock

ReturnFn_cons=@(d,aprime,a,r,w,kappa_j,varphi,agej,Jr,pension) EZReturnFn_cons_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,varphi,agej,Jr,pension);
ReturnFn_posU=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension) EZReturnFn_positiveUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
ReturnFn_negU=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension) EZReturnFn_negativeUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
...
clear V3 V4 Policy3 Policy4 V3fromPolicy
...
% consumption-units
ReturnFn_cons_scaled=@(d,aprime,a,r,w,kappa_j,varphi,agej,Jr,pension,ezscalefactor) ezscalefactor*EZReturnFn_cons_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=0;
vfoptions1.EZriskaversion='ezgamma';
...
ReturnFn_posU_scaled=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru) ezscalefactoru*EZReturnFn_positiveUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=1;
...
ReturnFn_negU_scaled=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru) ezscalefactoru*EZReturnFn_negativeUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=vfoptions;
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=0;
...
for ezcase=1:3
    ...
    fprintf('survivalprobability all-ones vs unset [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1a(:)-V1b(:))))
...
fprintf('EZ with EZriskaversion=0 and sj [EZ positive utils]: should give zero: %.3e \n',max(abs(V2a(:)-V2b(:))))
fprintf('EZ with EZriskaversion=0 and sj [EZ positive utils]: should give zero: %.3e \n',max(abs(Policy2a(:)-Policy2b(:))))
...
fprintf('EZ with EZriskaversion=0 and sj [EZ negative utils]: should give zero: %.3e \n',max(abs(V3a(:)-V3b(:))))
fprintf('EZ with EZriskaversion=0 and sj [EZ negative utils]: should give zero: %.3e \n',max(abs(Policy3a(:)-Policy3b(:))))
...
for ezcase=1:3
    ...
    [V4,Policy4]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,[],vfoptions4);
    V1fromPolicy=ValueFnFromPolicy_FHorz(Policy1,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,vfoptions1);
    fprintf('sj, ValueFnFromPolicy [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1fromPolicy(:)-V1(:))))
    fprintf('sj, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V2(:))))
    fprintf('sj, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy1(:)-Policy2(:))))
    fprintf('sj, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V3(:)-V4(:))))
...
for ezcase=1:3
    ...
    fprintf('warm-glow terminal-only, ValueFnFromPolicy [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1fromPolicy(:)-V1(:))))
    fprintf('warm-glow terminal-only, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V2(:))))
    fprintf('warm-glow terminal-only, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy1(:)-Policy2(:))))
    fprintf('warm-glow terminal-only, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V3(:)-V4(:))))
    fprintf('warm-glow terminal-only, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy3(:)-Policy4(:))))
    ...
    fprintf('warm-glow terminal-only vs explicit sjterm [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V5(:))))
...
for ezcase=1:3
    ...
    [V4,Policy4]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,[],vfoptions4);
    V1fromPolicy=ValueFnFromPolicy_FHorz(Policy1,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,vfoptions1);
    fprintf('warm-glow with sj, ValueFnFromPolicy [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1fromPolicy(:)-V1(:))))
    fprintf('warm-glow with sj, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V2(:))))
    fprintf('warm-glow with sj, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy1(:)-Policy2(:))))
    fprintf('warm-glow with sj, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V3(:)-V4(:))))
    fprintf('warm-glow with sj, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy3(:)-Policy4(:))))
end
clear V1 V2 V3 V4 Policy1 Policy2 Policy3 Policy4 V1fromPolicy
...
Params.ezphi=1/Params.ezgamma;
Params.ezsigma=Params.ezgamma;
ReturnFn_consWGoracle=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,beta,oneminussj,wg1,wg2) EZReturnFn_negativeUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension)+beta*oneminussj*((EZWarmGlowFn_cons(aprime,wg1,wg2)^(1-ezsigma))/(1-ezsigma));
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=0;
vfoptions1.EZriskaversion='ezgamma';
...
V1btransformed=((1-Params.ezgamma)*V1b).^(1/(1-Params.ezgamma));
fprintf('EZ gamma=1/phi collapse, sj+warm-glow, Policy [EZ cons-units]: should give zero: %.3e \n',max(abs(Policy1a(:)-Policy1b(:))))
...
ezrisk_store=Params.ezrisk;
Params.ezrisk=0;
ReturnFn_posUWGoracle=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,beta,oneminussj,wg1,wg2,wg3) EZReturnFn_positiveUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension)+beta*oneminussj*EZWarmGlowFn_positiveUtils(aprime,wg1,wg2,wg3);
ReturnFn_negUWGoracle=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,beta,oneminussj,wg1,wg2,wg3) EZReturnFn_negativeUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension)+beta*oneminussj*EZWarmGlowFn_negativeUtils(aprime,wg1,wg2,wg3);
% positive-valued utility fn
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
...
fprintf('EZ with EZriskaversion=0, sj+warm-glow [EZ positive utils]: should give zero: %.3e \n',max(abs(V2a(:)-V2b(:))))
fprintf('EZ with EZriskaversion=0, sj+warm-glow [EZ positive utils]: should give zero: %.3e \n',max(abs(Policy2a(:)-Policy2b(:))))
% negative-valued utility fn
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
...
fprintf('EZ with EZriskaversion=0, sj+warm-glow [EZ negative utils]: should give zero: %.3e \n',max(abs(V3a(:)-V3b(:))))
fprintf('EZ with EZriskaversion=0, sj+warm-glow [EZ negative utils]: should give zero: %.3e \n',max(abs(Policy3a(:)-Policy3b(:))))
Params.ezrisk=ezrisk_store;
clear V2a V2b V3a V3b Policy2a Policy2b Policy3a Policy3b
...
for ezcase=1:3
    ...
    Policybase=Policybase(:,:,1:Njs);
    [Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
    fprintf('V_Jplus1 with sj+warm-glow (jstar=%i) [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Vbase(:)-Vshort(:))))
    fprintf('V_Jplus1 with sj+warm-glow (jstar=%i) [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Policybase(:)-Policyshort(:))))
    ...
    Policybase=Policybase(:,:,1:Njs);
    [Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
    fprintf('V_Jplus1 with sj+warm-glow (jstar=%i, with DC) [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Vbase(:)-Vshort(:))))
    fprintf('V_Jplus1 with sj+warm-glow (jstar=%i, with DC) [EZ %s], this should be zero: %.3e \n',jstar,casestr,max(abs(Policybase(:)-Policyshort(:))))
end
clear Vbase Vshort Policybase Policyshort
...
% (utility is increasing in c, and a_grid(1)=0) and d=0 (retirement
% income does not depend on d, and the composite (c^varphi)*((1-d)^(1-varphi)) is decreasing in d,
% so the terminal composite is c^varphi). V and Policy at ages 1,...,N_j-1 must then
...
WGterm_cons=@(aprime,varphi,r,pension) ((1+r)*aprime+pension)^varphi; % the terminal composite itself (cons-units terminal V=(ezc1*F^ezc2)^ezc7=F)
WGterm_posU=@(aprime,ezsigma,varphi,r,pension) ((1+(((1+r)*aprime+pension)^varphi))^(1-ezsigma)-1)/(1-ezsigma);
WGterm_negU=@(aprime,ezsigma,varphi,r,pension) ((((1+r)*aprime+pension)^varphi)^(1-ezsigma))/(1-ezsigma);
Njs2=N_j-1;
Paramsjs=Params;
...
fprintf('N_j-1 terminal warm-glow identity [EZ cons-units], this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
...
fprintf('N_j-1 terminal warm-glow identity [EZ positive utils], this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('N_j-1 terminal warm-glow identity [EZ positive utils], this should be zero: %.3e \n',max(abs(Policybase(:)-Policyshort(:))))
% negative-valued utility fn
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
...
fprintf('N_j-1 terminal warm-glow identity [EZ negative utils], this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('N_j-1 terminal warm-glow identity [EZ negative utils], this should be zero: %.3e \n',max(abs(Policybase(:)-Policyshort(:))))
clear Vbase Vshort Policybase Policyshort
...
for ezcase=1:3
    ...
    [V1b,Policy1b]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params2,DiscountFactorParamNames,[],vfoptionsv);
    fprintf('EZmortalityriskaversion equal to own risk aversion vs unset [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1a(:)-V1b(:))))
    fprintf('EZmortalityriskaversion equal to own risk aversion vs unset [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy1a(:)-Policy1b(:))))
end
clear V1a V1b Policy1a Policy1b
...
for ezcase=1:3
    ...
    [V4,Policy4]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,[],vfoptions4);
    V1fromPolicy=ValueFnFromPolicy_FHorz(Policy1,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,vfoptions1);
    fprintf('EZmortalityriskaversion distinct, ValueFnFromPolicy [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1fromPolicy(:)-V1(:))))
    fprintf('EZmortalityriskaversion distinct, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V1(:)-V2(:))))
    fprintf('EZmortalityriskaversion distinct, basic vs DC [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy1(:)-Policy2(:))))
    fprintf('EZmortalityriskaversion distinct, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(V3(:)-V4(:))))
    fprintf('EZmortalityriskaversion distinct, GI vs DC+GI [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policy3(:)-Policy4(:))))
end
clear V1 V2 V3 V4 Policy1 Policy2 Policy3 Policy4 V1fromPolicy
...
% consumption-units
ReturnFn_cons_scaled2=@(d,aprime,a,r,w,kappa_j,varphi,agej,Jr,pension,ezscalefactor2) ezscalefactor2*EZReturnFn_cons_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,varphi,agej,Jr,pension);
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=0;
vfoptions1.EZriskaversion='ezgamma';
...
fprintf('EZoneminusbeta=2 vs manual (1-sj*beta) scaling [EZ cons-units]: should give zero: %.3e \n',max(abs(V4a(:)-V4b(:))))
fprintf('EZoneminusbeta=2 vs manual (1-sj*beta) scaling [EZ cons-units]: should give zero: %.3e \n',max(abs(Policy4a(:)-Policy4b(:))))

% utility-units, positive
ReturnFn_posU_scaled2=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru2) ezscalefactoru2*EZReturnFn_positiveUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=1;
...
fprintf('EZoneminusbeta=2 vs manual (1-sj*beta) scaling [EZ positive utils]: should give zero: %.3e \n',max(abs(V5a(:)-V5b(:))))
fprintf('EZoneminusbeta=2 vs manual (1-sj*beta) scaling [EZ positive utils]: should give zero: %.3e \n',max(abs(Policy5a(:)-Policy5b(:))))

% utility-units, negative
ReturnFn_negU_scaled2=@(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension,ezscalefactoru2) ezscalefactoru2*EZReturnFn_negativeUtils_d_noz_noe_nosemiz(d,aprime,a,r,w,kappa_j,ezsigma,varphi,agej,Jr,pension);
vfoptions1=struct();
vfoptions1.exoticpreferences='EpsteinZin';
vfoptions1.EZutils=1;
vfoptions1.EZpositiveutility=0;
...
fprintf('EZoneminusbeta=2 vs manual (1-sj*beta) scaling [EZ negative utils]: should give zero: %.3e \n',max(abs(V6a(:)-V6b(:))))
fprintf('EZoneminusbeta=2 vs manual (1-sj*beta) scaling [EZ negative utils]: should give zero: %.3e \n',max(abs(Policy6a(:)-Policy6b(:))))
...
% certainty-equivalent transform (ezc4*V')^ezc5 raises V' to a negative power, so a stray
% 0/-Inf V' would become Inf where it should stay an excluded -Inf. (With no shocks the CE is
% degenerate, so there is no expectation to contaminate across shock realizations; but the
% same mask code runs in the noz raws, and a broken marker still corrupts the max over
% aprime.) None of sections (i)-(ix) deliberately plants infeasible states. Here a_grid is
% extended upward with 3 points {8,9,10} that NO state in the ORIGINAL grid range can afford,
% at any age:
%   working age: c=(1+r)*a+w*kappa_j*d-aprime, and resources are at most 1.05*5+1*1*1=6.25
%     (and d<1 is required for feasibility anyway);
%   retirement: c=(1+r)*a+pension-aprime, and resources are at most 1.05*5+0.5=5.75.
% So aprime=8 (the smallest extended point) forces c<0, i.e. F=-Inf, from every a in the
% original range. The feasible choice set of every original-range state is therefore
...
% cases, then a cons-units repeat with sj+warm-glow. (Every solve prints the expected
% no-shocks warning.)
a_grid_ext=[a_grid; 8; 9; 10]; % three points above the max feasible resources (6.25) of the original range
n_a_ext=n_a+3;
maxresources=(1+Params.r)*a_grid(end)+max(Params.w*max(Params.kappa_j)*d_grid(end),Params.pension); % resources upper bound over the original range (d_grid(end)=1 overstates, as d<1 is required for feasibility)
...
for ezcase=1:3
    ...
    fprintf('infeasible-choice contamination, V on original grid [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Vbase(:)-Vexttrim(:))))
    fprintf('infeasible-choice contamination, Policy on original grid [EZ %s], this should be zero: %.3e \n',casestr,max(abs(Policybase(:)-Policyexttrim(:))))
...
Vexttrim=Vext(1:n_a,:);
Policyexttrim=Policyext(:,1:n_a,:);
fprintf('infeasible-choice contamination with sj+warm-glow, V on original grid [EZ cons-units], this should be zero: %.3e \n',max(abs(Vbase(:)-Vexttrim(:))))
fprintf('infeasible-choice contamination with sj+warm-glow, Policy on original grid [EZ cons-units], this should be zero: %.3e \n',max(abs(Policybase(:)-Policyexttrim(:))))
