...
%% ===== Section J: AutoCorrTransProbs_FHorz with e shocks =====
fprintf('\n-- Section J: AutoCorrTransProbs_FHorz (e shocks) --\n')
...
FnsToEvaluate_acp.zvar=@(d,aprime,a,z,e) z;
FnsToEvaluate_acp.evar=@(d,aprime,a,z,e) e;
...
% T3 (evar): e is iid, so Cov(e_j,e_{j+1}) = 0 exactly
err=max(abs(gather(ACP.evar.AutoCovariance)));
fprintf('T3 evar (iid) AutoCovariance, should be zero: %2.10f\n',err); fail_count=fail_count+(err>TOL_EXACT);
ac_e=gather(ACP.evar.AutoCorrelation);
err=max(abs(ac_e(~isnan(ac_e)))); % age-1 e is degenerate (point-mass jequaloneDist) so AutoCorr(1) is NaN
fprintf('T3 evar (iid) AutoCorrelation, should be zero: %2.10f\n',err); fail_count=fail_count+(err>TOL_EXACT);
