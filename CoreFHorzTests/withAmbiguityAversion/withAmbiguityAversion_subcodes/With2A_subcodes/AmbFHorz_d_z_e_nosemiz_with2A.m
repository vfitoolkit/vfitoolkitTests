function output=AmbFHorz_d_z_e_nosemiz_with2A(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
jequaloneDist(1,1,ceil(n_z/2),ceil(vfoptions.n_e/2))=1; % no assets, midpoint shocks
...
vfoptions.ambiguity_pi_z=vfoptionsbaseline.ambiguity_pi_z;
vfoptions.ambiguity_pi_e=vfoptionsbaseline.ambiguity_pi_e;
% The regular pi_z and vfoptions.pi_e (used for the agent distribution) equal prior 1, so no consistency warning
...
% lowmemory2 (model has both z and e, so lowmemory=2 is also valid)
