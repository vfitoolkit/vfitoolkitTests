function output=QHDFHorz_d1_z_e_semiz_with2A(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
ReturnFn=@(d1,d2,a1prime,a2prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost,phi1,phi2) ReturnFn_d1_z_e_semiz_with2A(d1,d2,a1prime,a2prime,a1,a2,semiz,z,e,r,w,kappa_j,sigma,agej,Jr,pension,eta,varphi,uempbenefit,searcheffortcost,phi1,phi2);

% Setup some FnsToEvaluate
FnsToEvaluate.assets=@(d1,d2,a1prime,a2prime,a1,a2,semiz,z,e) a1;
FnsToEvaluate.house=@(d1,d2,a1prime,a2prime,a1,a2,semiz,z,e) a2;
FnsToEvaluate.earnings=@(d1,d2,a1prime,a2prime,a1,a2,semiz,z,e,w,kappa_j) w*kappa_j*d1*semiz*z*e;
...
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Naive), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive, Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Naive), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive, Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Naive (with DC)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive (with DC)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive (with DC), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Naive (with DC)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive (with DC)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive (with DC), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Naive (with GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive (with GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive (with GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Naive (with GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive (with GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive (with GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Naive (with DC+GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive (with DC+GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Naive (with DC+GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Naive (with DC+GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive (with DC+GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Naive (with DC+GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Sophisticated), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated, Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Sophisticated), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated, Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with DC)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with DC)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with DC), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with DC)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with DC)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with DC), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=2;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with DC+GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with DC+GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=2 (Sophisticated (with DC+GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort,Valtshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with DC+GI)), this should be zero: %2.8f \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with DC+GI)), this should be zero: %2.8f \n',max(abs(Policybase(:)-Policyshort(:))))
fprintf('V_Jplus1, lowmemory=3 (Sophisticated (with DC+GI), Valt), this should be zero: %2.8f \n',max(abs(Valtbase(:)-Valtshort(:))))
vfoptionsjs.lowmemory=0;
