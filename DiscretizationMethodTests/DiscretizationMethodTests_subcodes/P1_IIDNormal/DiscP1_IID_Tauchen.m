function output=DiscP1_IID_Tauchen(calib,znums,figure_c)
% P1: discretizeIID_Tauchen, run at DEFAULTS only
%
% This command is not swept here. It is currently an exact copy of discretizeIIDNormal_Tauchen,
% which owns the option sweep, and the plan is for discretizeIID_Tauchen to be generalized to
...
fprintf('\n========== P1: discretizeIID_Tauchen (defaults only) ========== \n')
...
for c_c=1:nz
    ...
    tic;
    [e_grid,pi_e]=discretizeIID_Tauchen(mew,sigma,enum,Tauchen_q,struct());
    twarm=toc;
...
title('discretizeIID_Tauchen: |error|','Interpreter','none'); xlabel('enum'); legend('mean','variance','Location','best')
