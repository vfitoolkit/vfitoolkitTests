function output=DiscP10_ARp_FarmerToda(calib,cn,figure_c)
% P10: discretizeARp_FarmerToda
%
...
fprintf('\n========== P10: discretizeARp_FarmerToda, %s ========== \n',calib.(cn).name)

output=struct();
Rho=calib.(cn).Rho; p=calib.(cn).p;
mew=calib.mew; sigma=calib.sigma;
T=calib.(cn).gauss; % the analytic truth for this calibration with gaussian innovations
if p==2
    znums=calib.znums2;
else
    znums=calib.znums3;
...
for c_c=1:nz
    ...
    tic;
    [z_grid,pi_z,oo]=discretizeARp_FarmerToda(mew,Rho,sigma,znum,fo);
    twarm=toc;
    ...
    fallback(c_c)=mean(gather(oo.nMoments_grid)<2); % 2 is this command's default nMoments
    fprintf('znum=%3i: states matching fewer than the requested 2 moments: %2.1f%% \n',znum,100*fallback(c_c))
    ...
    fprintf('znum=%3i: otheroutputs.mewz is mew/(1-sum(Rho)) [T1], this should be zero: %2.8e \n',znum,abs(gather(oo.mewz)-T.mewz))
    ...
    fprintf('znum=%3i: skewness %+2.4f against truth %+2.4f, excess kurtosis %+2.4f against truth %+2.4f \n',znum,sk,T.skew,ek,T.exkurt)
...
figure(figure_c)
subplot(1,2,1)
semilogy(znums,max(err_mean,1e-18),'o-',znums,max(err_var,1e-18),'s-',znums,max(err_ac,1e-18),'^-')
set(gca,'XScale','log')
xlabel('znum per dimension'); ylabel('|error|')
title(['ARp\_FarmerToda: ',calib.(cn).name]); legend('mean','variance','autocorr','Location','best')
subplot(1,2,2)
loglog(znums,runtime,'o-')
xlabel('znum per dimension'); ylabel('seconds'); title('runtime')
sgtitle(['P10: discretizeARp\_FarmerToda, ',calib.(cn).name])
