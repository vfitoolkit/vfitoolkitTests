function output=CoreFHorzGPExpAsset_nod1_z_e_nosemiz(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
% Gul-Pesendorfer experienceasset, single standard endogenous asset a1: no d1, z, e.
...
vfoptions.temptationFn=@(d2,a1prime,a1,a2,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension) GPTemptationFn_nod1_z_e_nosemiz(d2,a1prime,a1,a2,z,e,lambdaGP,shiftGP,r,w,kappa_j,sigma,agej,Jr,pension);
...
fprintf('lowmemory=2 (base), this should be zero: %.3e \n',max(abs(V1(:)-V1C(:))))
fprintf('lowmemory=2 (base, Policy), this should be zero: %.3e \n',max(abs(Policy1(:)-Policy1C(:))))
...
fprintf('lowmemory=2 (DC1), this should be zero: %.3e \n',max(abs(V2(:)-V2C(:))))
fprintf('lowmemory=2 (DC1, Policy), this should be zero: %.3e \n',max(abs(Policy2(:)-Policy2C(:))))
...
fprintf('lowmemory=2 (GI1), this should be zero: %.3e \n',max(abs(V3(:)-V3C(:))))
fprintf('lowmemory=2 (GI1, Policy), this should be zero: %.3e \n',max(abs(Policy3(:)-Policy3C(:))))
...
fprintf('lowmemory=2 (DC1_GI1), this should be zero: %.3e \n',max(abs(V4(:)-V4C(:))))
fprintf('lowmemory=2 (DC1_GI1, Policy), this should be zero: %.3e \n',max(abs(Policy4(:)-Policy4C(:))))
