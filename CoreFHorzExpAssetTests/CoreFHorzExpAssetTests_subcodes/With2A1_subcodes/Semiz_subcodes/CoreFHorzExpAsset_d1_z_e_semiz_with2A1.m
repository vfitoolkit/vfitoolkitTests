function output=CoreFHorzExpAsset_d1_z_e_semiz_with2A1(n_d,n_a,n_a_big,n_z,N_j,d_grid,a_grid,a_grid_big,z_grid,pi_z,Params,DiscountFactorParamNames,AgeWeightParamNames,vfoptionsbaseline,simoptionsbaseline,figure_c)
...
jequaloneDist(1,1,2,ceil(vfoptions.n_semiz/2),ceil(n_z/2),ceil(vfoptions.n_e/2))=1;

ReturnFn=@(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e,r,r2,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost) ReturnFn_d1_z_e_semiz_with2A1(d1,d2,d3,a1prime,a1_2prime,a1,a1_2,a2,semiz,z,e,r,r2,w,kappa_j,sigma,varphi,eta,agej,Jr,pension,uempbenefit,searcheffortcost);
...
vfoptions1b=vfoptions1; vfoptions1b.divideandconquer=1; % base (no DC) brute-forces N_a1^2 on the fine 2-asset grid -> OOM; use DC as the exact fine-grid reference
[V1b,Policy1b]=ValueFnIter_Case1_FHorz(n_d,n_a_big,n_z,N_j,d_grid,a_grid_big,z_grid,pi_z,ReturnFn,Params,DiscountFactorParamNames,[],vfoptions1b);
...
jequaloneDist5=zeros([n_a,vfoptions.n_semiz,n_z,vfoptions.n_e],'gpuArray'); % small-grid init for Policy5
jequaloneDist5(1,1,2,ceil(vfoptions.n_semiz/2),ceil(n_z/2),ceil(vfoptions.n_e/2))=1;
...
[Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3, this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3, this should be zero: %.3e \n',max(abs(Policybase(:)-Policyshort(:))))
...
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (with DC), this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (with DC), this should be zero: %.3e \n',max(abs(Policybase(:)-Policyshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (with GI), this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (with GI), this should be zero: %.3e \n',max(abs(Policybase(:)-Policyshort(:))))
vfoptionsjs.lowmemory=0;
...
vfoptionsjs.lowmemory=3;
[Vshort,Policyshort]=ValueFnIter_Case1_FHorz(n_d,n_a,n_z,Njs,d_grid,a_grid,z_grid,pi_z,ReturnFn,Paramsjs,DiscountFactorParamNames,[],vfoptionsjs);
fprintf('V_Jplus1, lowmemory=3 (with DC+GI), this should be zero: %.3e \n',max(abs(Vbase(:)-Vshort(:))))
fprintf('V_Jplus1, lowmemory=3 (with DC+GI), this should be zero: %.3e \n',max(abs(Policybase(:)-Policyshort(:))))
vfoptionsjs.lowmemory=0;
